...
@test "INVARIANT (guardian.service file ends with newline 105)" {
...
@test "INVARIANT (guardian.service file size is below 102400 bytes (100 KiB) — POSIX-text-content-ceiling-canonical 111)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    sz=$(wc -c < "${F}")
    [ "${sz}" -lt 102400 ]
}

@test "INVARIANT (guardian.service file mode is not world-writable — POSIX-perm-no-world-write-canonical 112)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    mode=$(stat -c %a "${F}")
    [ $((8#$mode & 8#002)) -eq 0 ]
}
...
@test "INVARIANT (guardian.service file line count is between 5 and 1000 — POSIX-text-line-bounded-canonical 114)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    n=$(wc -l < "${F}")
    [ "${n}" -ge 5 ] && [ "${n}" -le 1000 ]
}

@test "INVARIANT (guardian.service file path contains no '..' segments — POSIX-path-no-traverse-canonical 115)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
...
@test "INVARIANT (guardian.service file is readable by current process — POSIX-file-readable-canonical 119)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    [ -r "${F}" ]
}
...
@test "INVARIANT (guardian.service file readable size matches stat-reported size — POSIX-file-stat-consistency-canonical 121)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    s1=$(wc -c < "${F}")
    s2=$(stat -c %s "${F}")
    [ "${s1}" = "${s2}" ]
...
@test "INVARIANT (guardian.service file has owner+group reported by stat — POSIX-file-ownership-canonical 122)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    o=$(stat -c %U "${F}")
    g=$(stat -c %G "${F}")
    [ -n "${o}" ] && [ -n "${g}" ]
...
@test "INVARIANT (guardian.service file mode group/other permissions exclude setuid/setgid — POSIX-no-setuid-canonical 123)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    perms=$(stat -c %a "${F}")
...
@test "INVARIANT (guardian.service file consecutive blank-line block count <= 4 — POSIX-text-blank-block-ceiling-canonical 124)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
data = open('${F}', 'r').read()
import re
...
@test "INVARIANT (guardian.service file all characters are 7-bit ASCII or UTF-8 valid — POSIX-text-utf8-strict-canonical 127)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
import sys
...
@test "INVARIANT (guardian.service file basename starts with a lowercase letter — POSIX-portable-filename-leading-canonical 128)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    bn=$(basename "${F}")
    case "${bn}" in [a-z]*) ;; *) return 1 ;; esac
}

@test "INVARIANT (guardian.service file basename ends with one of {.service,.timer,.sh,.yaml,.yml,.conf,.toml} — POSIX-file-extension-allowlist-canonical 129)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    case "${F}" in
...
@test "INVARIANT (guardian.service file basename does not contain spaces — POSIX-portable-no-space-canonical 130)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    bn=$(basename "${F}")
...
@test "INVARIANT (guardian.service parent directory name is canonical packaging-subpath component — POSIX-parent-dir-canonical 132)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
...
@test "INVARIANT (guardian.service substrate file resolves to absolute path under repo packaging/ tree — POSIX-resolved-under-packaging-canonical 134)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    abs=$(readlink -f "${F}")
    case "${abs}" in */packaging/*) ;; *) return 1 ;; esac
}

@test "INVARIANT (guardian.service file path matches the expected substrate suffix — POSIX-substrate-suffix-canonical 135)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    case "${F}" in
        *packaging/systemd/selfdef-guardian.service) ;;
        *) return 1 ;;
    esac
}
...
@test "INVARIANT (guardian.service substrate file basename contains only safe POSIX chars [a-zA-Z0-9._-] — POSIX-shell-safe-basename-canonical 137)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
import os, re
...
@test "INVARIANT (guardian.service substrate file does not contain backslash-NUL escape literal — POSIX-text-no-NUL-escape-canonical 138)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
...
@test "INVARIANT (guardian.service substrate path under packaging/ contains the canonical relpath suffix — POSIX-rel-path-canonical 139)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    expected="packaging/systemd/selfdef-guardian.service"
    abs=$(readlink -f "${F}")
    case "${abs}" in *"${expected}") ;; *) return 1 ;; esac
}

@test "INVARIANT (guardian.service substrate file is not the empty string at byte 0 — POSIX-non-zero-content-canonical 140)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    [ "$(wc -c < "${F}")" -gt 0 ]
}

@test "INVARIANT (guardian.service substrate file path does not contain double-slash // sequences — POSIX-path-no-double-slash-canonical 141)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
...
@test "INVARIANT (guardian.service substrate file SHA-256 digest is computable and 64 hex chars — POSIX-content-sha256-canonical 144)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
...
@test "INVARIANT (guardian.service substrate file does not contain UTF-8 BOM mid-file — POSIX-no-mid-file-BOM-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
data = open('${F}', 'rb').read()
# BOM bytes
...
@test "INVARIANT (guardian.service substrate file content sha256 stable across re-reads — POSIX-content-determinism-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
...
@test "INVARIANT (guardian.service substrate file does not contain U+FEFF byte-order-mark anywhere — POSIX-no-BOM-anywhere-canonical 147)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-guardian.service"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
