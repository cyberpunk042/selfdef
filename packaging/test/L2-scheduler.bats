...
@test "INVARIANT (scheduler.service file does not contain trailing whitespace — POSIX-text-canonical 106)" {
...
@test "INVARIANT (scheduler.service file is a regular file (not symlink, not device) — POSIX-file-type-canonical 109)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    [ -f "${F}" ] && [ ! -L "${F}" ]
}
...
@test "INVARIANT (scheduler.service file mode is not world-writable — POSIX-perm-no-world-write-canonical 112)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    mode=$(stat -c %a "${F}")
    [ $((8#$mode & 8#002)) -eq 0 ]
}

@test "INVARIANT (scheduler.service file extension is .service — POSIX-file-extension-canonical 113)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    case "${F}" in *.service) ;; *) return 1 ;; esac
}

@test "INVARIANT (scheduler.service file line count is between 5 and 1000 — POSIX-text-line-bounded-canonical 114)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    n=$(wc -l < "${F}")
    [ "${n}" -ge 5 ] && [ "${n}" -le 1000 ]
}
...
@test "INVARIANT (scheduler.service file contains no bare CR (no classic-Mac \r line endings) — POSIX-no-bare-CR-canonical 116)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
data = open('${F}', 'rb').read()
...
@test "INVARIANT (scheduler.service file contains at least 1 printable non-whitespace character — POSIX-text-printable-floor-canonical 117)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    LC_ALL=C grep -qP "[!-~]" "${F}"
}
...
@test "INVARIANT (scheduler.service file is readable by current process — POSIX-file-readable-canonical 119)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    [ -r "${F}" ]
}
...
@test "INVARIANT (scheduler.service file has owner+group reported by stat — POSIX-file-ownership-canonical 122)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    o=$(stat -c %U "${F}")
    g=$(stat -c %G "${F}")
    [ -n "${o}" ] && [ -n "${g}" ]
...
@test "INVARIANT (scheduler.service file mode group/other permissions exclude setuid/setgid — POSIX-no-setuid-canonical 123)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    perms=$(stat -c %a "${F}")
...
@test "INVARIANT (scheduler.service file consecutive blank-line block count <= 4 — POSIX-text-blank-block-ceiling-canonical 124)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
data = open('${F}', 'r').read()
import re
...
@test "INVARIANT (scheduler.service file has at least one non-blank line — POSIX-text-non-blank-floor-canonical 125)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    grep -qE '[^[:space:]]' "${F}"
}
...
@test "INVARIANT (scheduler.service file all characters are 7-bit ASCII or UTF-8 valid — POSIX-text-utf8-strict-canonical 127)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
import sys
...
@test "INVARIANT (scheduler.service file basename ends with one of {.service,.timer,.sh,.yaml,.yml,.conf,.toml} — POSIX-file-extension-allowlist-canonical 129)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    case "${F}" in
...
@test "INVARIANT (scheduler.service file basename does not contain spaces — POSIX-portable-no-space-canonical 130)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    bn=$(basename "${F}")
    case "${bn}" in *' '*) return 1 ;; *) ;; esac
}

@test "INVARIANT (scheduler.service file basename does not start with a dot (no hidden-file convention) — POSIX-no-hidden-canonical 131)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    bn=$(basename "${F}")
    case "${bn}" in .*) return 1 ;; *) ;; esac
}

@test "INVARIANT (scheduler.service parent directory name is canonical packaging-subpath component — POSIX-parent-dir-canonical 132)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    pdir=$(dirname "${F}" | xargs basename)
    case "${pdir}" in systemd|scripts|lib|tetragon-policies) ;; *) return 1 ;; esac
}

@test "INVARIANT (scheduler.service file path resolves to an existing entry — POSIX-file-exists-resolved-canonical 133)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    abs=$(readlink -f "${F}")
    [ -e "${abs}" ]
}
...
@test "INVARIANT (scheduler.service file path matches the expected substrate suffix — POSIX-substrate-suffix-canonical 135)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    case "${F}" in
        *packaging/systemd/selfdef-scheduler.service) ;;
        *) return 1 ;;
    esac
}
...
@test "INVARIANT (scheduler.service substrate file does not contain backslash-NUL escape literal — POSIX-text-no-NUL-escape-canonical 138)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
...
@test "INVARIANT (scheduler.service substrate path under packaging/ contains the canonical relpath suffix — POSIX-rel-path-canonical 139)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    expected="packaging/systemd/selfdef-scheduler.service"
    abs=$(readlink -f "${F}")
    case "${abs}" in *"${expected}") ;; *) return 1 ;; esac
}

@test "INVARIANT (scheduler.service substrate file is not the empty string at byte 0 — POSIX-non-zero-content-canonical 140)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    [ "$(wc -c < "${F}")" -gt 0 ]
}
...
@test "INVARIANT (scheduler.service substrate file md5 sum is deterministic (file content is stable for hash computation) — POSIX-content-hashable-canonical 143)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
...
@test "INVARIANT (scheduler.service substrate file SHA-256 digest is computable and 64 hex chars — POSIX-content-sha256-canonical 144)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    h=$(sha256sum "${F}" | cut -d' ' -f1)
    [ "${#h}" = "64" ]
}

@test "INVARIANT (scheduler.service substrate file does not contain UTF-8 ZWSP (U+200B) — POSIX-no-zero-width-space-canonical 145)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
...
@test "INVARIANT (scheduler.service substrate file does not contain UTF-8 BOM mid-file — POSIX-no-mid-file-BOM-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
data = open('${F}', 'rb').read()
# BOM bytes
...
@test "INVARIANT (scheduler.service substrate file content sha256 stable across re-reads — POSIX-content-determinism-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
...
@test "INVARIANT (scheduler.service substrate file does not contain U+FEFF byte-order-mark anywhere — POSIX-no-BOM-anywhere-canonical 147)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-scheduler.service"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
