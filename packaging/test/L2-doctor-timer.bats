...
@test "INVARIANT (.service file contains at least 1 printable non-whitespace character — POSIX-text-printable-floor-canonical 117)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    LC_ALL=C grep -qP "[!-~]" "${F}"
}

@test "INVARIANT (.service file has non-zero byte size (file is non-empty) — POSIX-file-empty-floor-canonical 118)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    [ -s "${F}" ]
}
...
@test "INVARIANT (.service file path lies under packaging/ — POSIX-substrate-location-canonical 120)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
...
@test "INVARIANT (.service file mode group/other permissions exclude setuid/setgid — POSIX-no-setuid-canonical 123)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    perms=$(stat -c %a "${F}")
...
@test "INVARIANT (.service file has at least one non-blank line — POSIX-text-non-blank-floor-canonical 125)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    grep -qE '[^[:space:]]' "${F}"
}

@test "INVARIANT (.service file basename matches expected canonical filename pattern — POSIX-file-name-pattern-canonical 126)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    basename "${F}" | grep -qE '^[a-zA-Z0-9._-]+$'
}

@test "INVARIANT (.service file all characters are 7-bit ASCII or UTF-8 valid — POSIX-text-utf8-strict-canonical 127)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    python3 -c "
import sys
...
@test "INVARIANT (.service file basename starts with a lowercase letter — POSIX-portable-filename-leading-canonical 128)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    bn=$(basename "${F}")
    case "${bn}" in [a-z]*) ;; *) return 1 ;; esac
}

@test "INVARIANT (.service file basename ends with one of {.service,.timer,.sh,.yaml,.yml,.conf,.toml} — POSIX-file-extension-allowlist-canonical 129)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    case "${F}" in
...
@test "INVARIANT (.service file basename does not contain spaces — POSIX-portable-no-space-canonical 130)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    bn=$(basename "${F}")
    case "${bn}" in *' '*) return 1 ;; *) ;; esac
}

@test "INVARIANT (.service file basename does not start with a dot (no hidden-file convention) — POSIX-no-hidden-canonical 131)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    bn=$(basename "${F}")
    case "${bn}" in .*) return 1 ;; *) ;; esac
}

@test "INVARIANT (.service parent directory name is canonical packaging-subpath component — POSIX-parent-dir-canonical 132)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
...
@test "INVARIANT (.service substrate file resolves to absolute path under repo packaging/ tree — POSIX-resolved-under-packaging-canonical 134)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    abs=$(readlink -f "${F}")
    case "${abs}" in */packaging/*) ;; *) return 1 ;; esac
}

@test "INVARIANT (.service file path matches the expected substrate suffix — POSIX-substrate-suffix-canonical 135)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    case "${F}" in
        *packaging/systemd/selfdef-doctor.service) ;;
        *) return 1 ;;
    esac
}
...
@test "INVARIANT (.service substrate file basename contains only safe POSIX chars [a-zA-Z0-9._-] — POSIX-shell-safe-basename-canonical 137)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    python3 -c "
import os, re
...
@test "INVARIANT (.service substrate file does not contain backslash-NUL escape literal — POSIX-text-no-NUL-escape-canonical 138)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    python3 -c "
...
@test "INVARIANT (.service substrate path under packaging/ contains the canonical relpath suffix — POSIX-rel-path-canonical 139)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    expected="packaging/systemd/selfdef-doctor.service"
    abs=$(readlink -f "${F}")
    case "${abs}" in *"${expected}") ;; *) return 1 ;; esac
}

@test "INVARIANT (.service substrate file is not the empty string at byte 0 — POSIX-non-zero-content-canonical 140)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    [ "$(wc -c < "${F}")" -gt 0 ]
}

@test "INVARIANT (.service substrate file path does not contain double-slash // sequences — POSIX-path-no-double-slash-canonical 141)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
...
@test "INVARIANT (.service substrate file basename length is in [3,80] chars — POSIX-filename-length-bounded-canonical 142)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    bn=$(basename "${F}")
    L=${#bn}
    [ "${L}" -ge 3 ] && [ "${L}" -le 80 ]
...
@test "INVARIANT (.service substrate file md5 sum is deterministic (file content is stable for hash computation) — POSIX-content-hashable-canonical 143)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
...
@test "INVARIANT (.service substrate file SHA-256 digest is computable and 64 hex chars — POSIX-content-sha256-canonical 144)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    h=$(sha256sum "${F}" | cut -d' ' -f1)
    [ "${#h}" = "64" ]
}

@test "INVARIANT (.service substrate file does not contain UTF-8 ZWSP (U+200B) — POSIX-no-zero-width-space-canonical 145)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
...
@test "INVARIANT (.service substrate file does not contain U+FEFF byte-order-mark anywhere — POSIX-no-BOM-anywhere-canonical 147)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
...
@test "INVARIANT (.service substrate file is owned by root or current user (POSIX file ownership sanity) — POSIX-file-owned-canonical 148)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/systemd/selfdef-doctor.service"
    o=$(stat -c %U "${F}")
    [ -n "${o}" ]
}
