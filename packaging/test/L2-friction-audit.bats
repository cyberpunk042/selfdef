...
@test "INVARIANT (script file size is below 102400 bytes (100 KiB) — POSIX-text-content-ceiling-canonical 111)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    sz=$(wc -c < "${F}")
    [ "${sz}" -lt 102400 ]
}
...
@test "INVARIANT (script file extension is .sh — POSIX-file-extension-canonical 113)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    case "${F}" in *.sh) ;; *) return 1 ;; esac
}

@test "INVARIANT (script file line count is between 5 and 1000 — POSIX-text-line-bounded-canonical 114)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    n=$(wc -l < "${F}")
    [ "${n}" -ge 5 ] && [ "${n}" -le 1000 ]
}

@test "INVARIANT (script file path contains no '..' segments — POSIX-path-no-traverse-canonical 115)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
...
@test "INVARIANT (script file contains no bare CR (no classic-Mac \r line endings) — POSIX-no-bare-CR-canonical 116)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
data = open('${F}', 'rb').read()
...
@test "INVARIANT (script file readable size matches stat-reported size — POSIX-file-stat-consistency-canonical 121)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    s1=$(wc -c < "${F}")
    s2=$(stat -c %s "${F}")
    [ "${s1}" = "${s2}" ]
...
@test "INVARIANT (script file mode group/other permissions exclude setuid/setgid — POSIX-no-setuid-canonical 123)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    perms=$(stat -c %a "${F}")
...
@test "INVARIANT (script file consecutive blank-line block count <= 4 — POSIX-text-blank-block-ceiling-canonical 124)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
data = open('${F}', 'r').read()
import re
...
@test "INVARIANT (script file basename matches expected canonical filename pattern — POSIX-file-name-pattern-canonical 126)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
...
@test "INVARIANT (script file basename starts with a lowercase letter — POSIX-portable-filename-leading-canonical 128)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    bn=$(basename "${F}")
...
@test "INVARIANT (script file path resolves to an existing entry — POSIX-file-exists-resolved-canonical 133)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    abs=$(readlink -f "${F}")
    [ -e "${abs}" ]
}

@test "INVARIANT (script substrate file resolves to absolute path under repo packaging/ tree — POSIX-resolved-under-packaging-canonical 134)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    abs=$(readlink -f "${F}")
...
@test "INVARIANT (script substrate file path matches the expected packaging/scripts/friction-audit.sh suffix — POSIX-substrate-path-canonical-strict 136)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    abs=$(readlink -f "${F}")
    expected_suffix="packaging/scripts/friction-audit.sh"
    case "${abs}" in *"${expected_suffix}") ;; *) return 1 ;; esac
}

@test "INVARIANT (script substrate file basename contains only safe POSIX chars [a-zA-Z0-9._-] — POSIX-shell-safe-basename-canonical 137)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
import os, re
...
@test "INVARIANT (script substrate file does not contain backslash-NUL escape literal — POSIX-text-no-NUL-escape-canonical 138)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
...
@test "INVARIANT (script substrate path under packaging/ contains the canonical relpath suffix — POSIX-rel-path-canonical 139)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    expected="packaging/scripts/friction-audit.sh"
    abs=$(readlink -f "${F}")
    case "${abs}" in *"${expected}") ;; *) return 1 ;; esac
}

@test "INVARIANT (script substrate file is not the empty string at byte 0 — POSIX-non-zero-content-canonical 140)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    [ "$(wc -c < "${F}")" -gt 0 ]
}
...
@test "INVARIANT (script substrate file md5 sum is deterministic (file content is stable for hash computation) — POSIX-content-hashable-canonical 143)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
...
@test "INVARIANT (script substrate file does not contain UTF-8 ZWSP (U+200B) — POSIX-no-zero-width-space-canonical 145)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
...
@test "INVARIANT (script substrate file does not contain UTF-8 BOM mid-file — POSIX-no-mid-file-BOM-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
    python3 -c "
data = open('${F}', 'rb').read()
# BOM bytes
...
@test "INVARIANT (script substrate file content sha256 stable across re-reads — POSIX-content-determinism-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/scripts/friction-audit.sh"
