...
@test "INVARIANT (YAML file basename ends with one of {.service,.timer,.sh,.yaml,.yml,.conf,.toml} — POSIX-file-extension-allowlist-canonical 129)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    case "${F}" in
...
@test "INVARIANT (YAML file basename does not start with a dot (no hidden-file convention) — POSIX-no-hidden-canonical 131)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    bn=$(basename "${F}")
    case "${bn}" in .*) return 1 ;; *) ;; esac
}
...
@test "INVARIANT (YAML file path matches the expected substrate suffix — POSIX-substrate-suffix-canonical 135)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    case "${F}" in
        *packaging/tetragon-policies/sovereign-perimeter.yaml) ;;
        *) return 1 ;;
    esac
}
...
@test "INVARIANT (YAML substrate file basename contains only safe POSIX chars [a-zA-Z0-9._-] — POSIX-shell-safe-basename-canonical 137)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    python3 -c "
import os, re
...
@test "INVARIANT (YAML substrate file path does not contain double-slash // sequences — POSIX-path-no-double-slash-canonical 141)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
...
@test "INVARIANT (YAML substrate file basename length is in [3,80] chars — POSIX-filename-length-bounded-canonical 142)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    bn=$(basename "${F}")
    L=${#bn}
    [ "${L}" -ge 3 ] && [ "${L}" -le 80 ]
...
@test "INVARIANT (YAML substrate file md5 sum is deterministic (file content is stable for hash computation) — POSIX-content-hashable-canonical 143)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
...
@test "INVARIANT (YAML substrate file does not contain UTF-8 ZWSP (U+200B) — POSIX-no-zero-width-space-canonical 145)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
...
@test "INVARIANT (YAML substrate file does not contain UTF-8 BOM mid-file — POSIX-no-mid-file-BOM-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    python3 -c "
data = open('${F}', 'rb').read()
# BOM bytes
...
@test "INVARIANT (YAML substrate file content sha256 stable across re-reads — POSIX-content-determinism-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
...
@test "INVARIANT (YAML substrate file does not contain U+FEFF byte-order-mark anywhere — POSIX-no-BOM-anywhere-canonical 147)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/tetragon-policies/sovereign-perimeter.yaml"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
