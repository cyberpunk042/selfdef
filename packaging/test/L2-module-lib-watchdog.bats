...
@test "INVARIANT (lib file ends with newline 105)" {
...
@test "INVARIANT (lib file size is below 102400 bytes (100 KiB) — POSIX-text-content-ceiling-canonical 111)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    sz=$(wc -c < "${F}")
    [ "${sz}" -lt 102400 ]
}

@test "INVARIANT (lib file mode is not world-writable — POSIX-perm-no-world-write-canonical 112)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    mode=$(stat -c %a "${F}")
    [ $((8#$mode & 8#002)) -eq 0 ]
}
...
@test "INVARIANT (lib file line count is between 5 and 1000 — POSIX-text-line-bounded-canonical 114)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    n=$(wc -l < "${F}")
    [ "${n}" -ge 5 ] && [ "${n}" -le 1000 ]
}

@test "INVARIANT (lib file path contains no '..' segments — POSIX-path-no-traverse-canonical 115)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
...
@test "INVARIANT (lib file contains no bare CR (no classic-Mac \r line endings) — POSIX-no-bare-CR-canonical 116)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
data = open('${F}', 'rb').read()
...
@test "INVARIANT (lib file contains at least 1 printable non-whitespace character — POSIX-text-printable-floor-canonical 117)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    LC_ALL=C grep -qP "[!-~]" "${F}"
}

@test "INVARIANT (lib file has non-zero byte size (file is non-empty) — POSIX-file-empty-floor-canonical 118)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    [ -s "${F}" ]
}
...
@test "INVARIANT (lib file path lies under packaging/ — POSIX-substrate-location-canonical 120)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
...
@test "INVARIANT (lib file has owner+group reported by stat — POSIX-file-ownership-canonical 122)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    o=$(stat -c %U "${F}")
    g=$(stat -c %G "${F}")
    [ -n "${o}" ] && [ -n "${g}" ]
...
@test "INVARIANT (lib file mode group/other permissions exclude setuid/setgid — POSIX-no-setuid-canonical 123)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    perms=$(stat -c %a "${F}")
...
@test "INVARIANT (lib file consecutive blank-line block count <= 4 — POSIX-text-blank-block-ceiling-canonical 124)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
data = open('${F}', 'r').read()
import re
...
@test "INVARIANT (lib file all characters are 7-bit ASCII or UTF-8 valid — POSIX-text-utf8-strict-canonical 127)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
import sys
...
@test "INVARIANT (lib file basename starts with a lowercase letter — POSIX-portable-filename-leading-canonical 128)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    bn=$(basename "${F}")
...
@test "INVARIANT (lib file basename does not contain spaces — POSIX-portable-no-space-canonical 130)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    bn=$(basename "${F}")
...
@test "INVARIANT (lib parent directory name is canonical packaging-subpath component — POSIX-parent-dir-canonical 132)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    pdir=$(dirname "${F}" | xargs basename)
    case "${pdir}" in systemd|scripts|lib|tetragon-policies) ;; *) return 1 ;; esac
}

@test "INVARIANT (lib file path resolves to an existing entry — POSIX-file-exists-resolved-canonical 133)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    abs=$(readlink -f "${F}")
    [ -e "${abs}" ]
}

@test "INVARIANT (lib substrate file resolves to absolute path under repo packaging/ tree — POSIX-resolved-under-packaging-canonical 134)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    abs=$(readlink -f "${F}")
    case "${abs}" in */packaging/*) ;; *) return 1 ;; esac
}

@test "INVARIANT (lib file path matches the expected substrate suffix — POSIX-substrate-suffix-canonical 135)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    case "${F}" in
        *packaging/lib/module-lib.sh) ;;
        *) return 1 ;;
    esac
}
...
@test "INVARIANT (lib substrate file does not contain backslash-NUL escape literal — POSIX-text-no-NUL-escape-canonical 138)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
...
@test "INVARIANT (lib substrate path under packaging/ contains the canonical relpath suffix — POSIX-rel-path-canonical 139)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    expected="packaging/lib/module-lib.sh"
    abs=$(readlink -f "${F}")
...
@test "INVARIANT (lib substrate file basename length is in [3,80] chars — POSIX-filename-length-bounded-canonical 142)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    bn=$(basename "${F}")
    L=${#bn}
    [ "${L}" -ge 3 ] && [ "${L}" -le 80 ]
...
@test "INVARIANT (lib substrate file md5 sum is deterministic (file content is stable for hash computation) — POSIX-content-hashable-canonical 143)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
...
@test "INVARIANT (lib substrate file SHA-256 digest is computable and 64 hex chars — POSIX-content-sha256-canonical 144)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
...
@test "INVARIANT (lib substrate file does not contain UTF-8 BOM mid-file — POSIX-no-mid-file-BOM-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
data = open('${F}', 'rb').read()
# BOM bytes
...
@test "INVARIANT (lib substrate file content sha256 stable across re-reads — POSIX-content-determinism-canonical 146)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
...
@test "INVARIANT (lib substrate file does not contain U+FEFF byte-order-mark anywhere — POSIX-no-BOM-anywhere-canonical 147)" {
    F="${BATS_TEST_DIRNAME}/../../packaging/lib/module-lib.sh"
    python3 -c "
data = open('${F}', 'r', encoding='utf-8', errors='replace').read()
