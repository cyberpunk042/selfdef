...
**Status:** implemented — operator chose **Option 1** (shell to system `openssl`, zero new crypto dependency; 2026-07-19). Two increments landed:
> 1. **Verify core** — `crates/selfdef-signing/src/ms003.rs`: envelope parse, `keyid`, `canonical_bytes` (recursive sorted-key form, robust to `preserve_order`), inline base64url, a selfdef-owned `TrustAnchors` store, and `verify_record` → the 5 `VerifyStatus` variants, all `openssl`-backed. 20 crate tests incl. a **cross-implementation golden** (a record signed by the real sovereign-os `scripts/lib/ms003.py` verifies here).
> 2. **Operator surface** — `selfdefctl ms003 {anchor-add,anchor-list,verify,sweep}` (`crates/selfdef-cli/src/ms003.rs`), verify-only, with shell-gate exit codes (0 verified / 2 security-event / 1 benign) and a `sweep --metrics` that folds per-status classification into **Layer-B Prometheus gauges** (`selfdef_ms003_records{status}` + `selfdef_ms003_security_events`) the selfdef alert engine consumes. 5 CLI tests + the L1-cli-surface count gate (`ms003` = 4). **F-2026-034 fully closed** on the selfdef side: selfdef now cryptographically verifies sovereign-os's signed mutation/decision records and surfaces tamper/untrusted-signer as an alertable security event.
