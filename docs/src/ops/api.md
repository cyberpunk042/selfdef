...
| GET    | `/v1/modules/:name/check`             | MS006/MS016..MS031: invokes `<modules_dir>/<name>/install/check.sh` per the docs/dev/modules.md module-author contract (check.sh is documented read-only). Returns `{module, script, exit_code, ok, stdout, stderr}` — stdout/stderr truncated at 64 KiB. Slug validated against `[a-z0-9-]{1,64}`. 404 if the module or its check.sh is missing. Gives operators per-module health visibility (e.g. `integrity-sentinel` baseline drift, `tetragon` policy load state) without shell access. |
...
| GET    | `/v1/storage`                         | MS011 Z-10 / SDD-026: storage state surface. Parses `df -P` per request (excludes tmpfs/devtmpfs/squashfs/etc. by default — `SELFDEF_STORAGE_INCLUDE_PSEUDO=1` to include); walks `/var/log/selfdef`, `/var/cache/selfdef`, `/var/lib/selfdef` for bytes + file counts. Returns `{worst, mounts: [...], log_dirs: [...]}`. Mount state thresholds: green (< 70 %), yellow (70-89 %), red (≥ 90 %). |
| GET    | `/v1/raid`                            | MS011 Z-9 / SDD-026: software RAID state from `/proc/mdstat` (read-only — selfdef NEVER manipulates the array). Returns `{worst, mdstat_present, arrays: [{name, level, members, health, state}]}`. State classified per level: raid0/linear → red on any missing; raid1/5/10 → yellow on 1, red on 2+; raid6 → yellow on 1-2, red on 3+. Empty `arrays` + `mdstat_present=false` = no MD support on this host. |
| GET    | `/v1/gpu`                             | MS011 Z-5 / SDD-026: per-GPU power-draw deviance. Runs `nvidia-smi --query-gpu=index,power.draw,power.limit` and reads `/etc/selfdef/gpu-policy.toml` (`SELFDEF_GPU_POLICY` to override). Classifies each GPU: green (draw within ±tolerance_watts of expected_power_limit_watts; default tolerance 25 W), yellow (1×-2× tolerance drift), red (>2× tolerance OR draw exceeds nvidia-smi power.limit), unknown (no operator policy entry for this GPU OR nvidia-smi N/A). Returns `{worst, policy_path, policy_present, gpus: [{index, current_watts, power_limit_watts, expected_limit_watts, tolerance_watts, state, detail}]}`. |
| GET    | `/v1/cpu`                             | MS011 Z-4 / SDD-026: CPU mode classification (read-only). Reads `/sys/devices/system/cpu/*/cpufreq/scaling_governor` + SMT state (`/sys/devices/system/cpu/smt/active` or `smt/control`). Classifies into one of: `ultra-low-power` (powersave + SMT off), `balanced` (schedutil OR ondemand + SMT on), `sustained-burst` (performance + SMT on), `peak-inference` (performance + SMT off), `custom` (anything else or mixed governors). Returns `{mode, governors, smt_enabled, cpufreq_present, smt_present}`. SET surface (operator radio-button) will land in a follow-up. |
| GET    | `/v1/health`                          | MS011 Z-6 / SDD-026: composite autohealth aggregate. Single endpoint for "is everything OK?" — calls each underlying probe (`/v1/alerts`, `/v1/network`, `/v1/storage`, `/v1/raid`, `/v1/gpu`, `/v1/cpu`), normalizes each into `ok/warn/critical/unknown`, returns `{worst, components: [{name, state, detail}]}`. The `cpu` row is informational (mode is operator choice, never degrades the aggregate). External monitoring + the daemon's autohealth scheduler consume this in preference to scraping every endpoint separately. |
| GET    | `/v1/audit-chains`                    | MS009: composite chain-check across the 3 chained-audit watchdogs (perimeter / guardian / scheduler). Runs each watchdog's `audit_chain_check` on its OCSF JSONL file. Returns `{worst, chains: [{watchdog, path, events_verified, ok, error}]}`. `worst` is `"ok"` iff every chain verified; `"critical"` if any chain broke. When a chain breaks, `events_verified` is 0 and `error` carries the verifier's line-number + detail string. Mirrors the CLI's `selfdefctl <watchdog> audit-cycle replay` verbs but surfaces them as one composite read so the dashboard + monitoring can probe all chains in a single HTTP call. |
| GET    | `/v1/commit-authority`                | MS041 / SDD-043 D-3: static commit-authority doctrine schema. Returns `{commit_types, mandatory_fields, policy_outcomes, rollback_statuses, high_risk_gate_fields, high_risk_classifier_rules, refusal_rules, doctrine_phrase}`. Agents / dashboards / external tooling consume this to learn the durable-change contract (8 commit types + 5 mandatory fields + 3 high-risk gates + F04871..F04875 classifier + F04852 refusal rules + verbatim R09601 doctrine phrase as drift detector) without reading the Rust source. Static — doctrine doesn't change at runtime. |
| GET    | `/v1/tool-authority`                  | MS042 / SDD-050 D-2: static tool-policy pipeline schema. Returns `{tool_ids, execution_modes, profiles, gate_pipeline, post_pipeline, refusal_rules}`. The 8 canonical ToolId variants × 7 ExecutionMode × 6 Profile authorization matrix + the 9-gate composition pipeline (admit → permits → version-pin → redact-args → audit → invoke → watchdog → admit_chunk → truncate) + post-pipeline shape-check + veil-wrap + refusal rules. Static. |
