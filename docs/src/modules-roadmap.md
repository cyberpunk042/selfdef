...
| `vpn-bridge`  | network       | shipping   | new                    | WireGuard overlay that survives **double-NAT** — both ends behind their own NAT, neither port-forwardable. v0.1.0 ships only the `relay-via-server` profile: operator stands up a small public WireGuard relay; both endpoints peer to it with `PersistentKeepalive`. STUN hole-punching and Tailscale/Headscale profiles are deferred. Module owns service + optional FORWARD nftables rules; **does not** render `wg.conf` (operator writes it; README shows the exact format). |
...
- `vpn-bridge` v0.2.0: STUN-assisted hole-punching profile (for cone-NAT setups where a public relay isn't available).
