...
| `agent-guard` | hardening     | shipping (v0.3.0) | new                    | Tetragon TracingPolicies for AI agents in Docker/Podman/containerd containers: `etc-write-guard`, `container-shell-guard`, `egress-guard` (with operator-defined CIDR allowlist), a forward-looking `securemessage-guard` stub, and `gpu-device-guard` (watches `/dev/nvidia*` etc. with an in-container binary allowlist). Two profiles: `audit` (Post-only) and `enforce` (Sigkill). Per-policy action overrides + per-policy enable/disable so operators ramp up gradually. Container scope = `container` (matchNamespaces, Pid != host_ns — every runtime) or `pod-label` (matchPodSelector — narrower, for k8s pods carrying an operator-defined label). |
