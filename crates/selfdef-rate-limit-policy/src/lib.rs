...
impl RateLimitPolicy {
    ...
    pub fn evaluate(
        ...
        let Some(c) = self.get(profile) else {
            return RateVerdict::Breach;
        };
        let max_ratio_pct = [
...
mod tests {
    ...
    fn within_below_80pct() {
        ...
        assert_eq!(
            p.evaluate(Profile::Private, 3, 30, 300),
            RateVerdict::Within
        );
    }
    ...
    fn breach_at_ceiling() {
        let p = RateLimitPolicy::canonical();
        assert_eq!(
            p.evaluate(Profile::Private, 5, 30, 300),
            RateVerdict::Breach
        );
        // Above ceiling.
        assert_eq!(
            p.evaluate(Profile::Private, 50, 30, 300),
            RateVerdict::Breach
        );
    }
    ...
    fn rpm_can_trigger_breach_alone() {
        ...
        assert_eq!(
            p.evaluate(Profile::Private, 1, 70, 100),
            RateVerdict::Breach
        );
    }
    ...
    fn verdict_serde_kebab() {
        assert_eq!(
            serde_json::to_string(&RateVerdict::Within).unwrap(),
            "\"within\""
        );
        assert_eq!(
            serde_json::to_string(&RateVerdict::Warn).unwrap(),
            "\"warn\""
        );
        assert_eq!(
            serde_json::to_string(&RateVerdict::Breach).unwrap(),
            "\"breach\""
        );
    }
