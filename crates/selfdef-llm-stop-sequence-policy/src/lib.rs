...
impl LlmStopSequencePolicy {
    ...
    pub fn canonical() -> Self {
        ...
        let canon: Vec<String> = vec!["<|im_end|>".into(), "<|endoftext|>".into(), "\n\n\n".into()];
        for &p in &[
            Profile::Private,
            Profile::Fast,
            Profile::Careful,
    ...
    pub fn add(&mut self, profile: Profile, sequence: &str) -> Result<(), StopError> {
        if sequence.is_empty() {
            return Err(StopError::EmptySequence);
        }
        self.sequences
            .entry(profile)
            .or_default()
...
mod tests {
    ...
    fn stop_sequence_found() {
        ...
        let stop = p
            .should_stop(Profile::Fast, "hello world<|im_end|>more")
            .unwrap();
    ...
    fn earliest_position_wins() {
        ...
        let stop = p
            .should_stop(Profile::Fast, "STOP-EARLY then <|im_end|>")
            .unwrap();
    ...
    fn longest_match_at_same_position() {
        let mut p = LlmStopSequencePolicy {
            schema_version: SCHEMA_VERSION.into(),
    ...
    fn empty_sequence_rejected() {
        let mut p = LlmStopSequencePolicy::canonical();
        assert!(matches!(
            p.add(Profile::Fast, "").unwrap_err(),
            StopError::EmptySequence
        ));
    }
