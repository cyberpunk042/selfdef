...
impl BundleLoadPolicy {
    ...
    pub fn decide(&self, b: &Bundle) -> LoadVerdict {
        if b.id.is_empty() { return LoadVerdict::EmptyId; }
        ...
        match b.source {
            BundleSource::Vendor => {
                if !b.signature_present { return LoadVerdict::VendorUnsigned; }
                LoadVerdict::Loaded
            }
...
mod tests {
    ...
    fn bundle(id: &str, src: BundleSource, loc: SourceLocation, signed: bool, approved: bool) -> Bundle {
        Bundle {
            id: id.into(),
            source: src,
            location: loc,
    ...
    fn vendor_must_be_signed() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Vendor, SourceLocation::Local, false, false);
    ...
    fn vendor_signed_loads() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Vendor, SourceLocation::Local, true, false);
    ...
    fn operator_local_unsigned_loads() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Operator, SourceLocation::Local, false, false);
        assert!(matches!(p.decide(&b), LoadVerdict::Loaded));
    }
    ...
    fn operator_network_unsigned_rejected() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Operator, SourceLocation::Network, false, false);
    ...
    fn operator_network_signed_loads() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Operator, SourceLocation::Network, true, false);
        assert!(matches!(p.decide(&b), LoadVerdict::Loaded));
    }
    ...
    fn untrusted_needs_signature_and_approval() {
        let p = BundleLoadPolicy::new();
        let b = bundle("a", BundleSource::Untrusted, SourceLocation::Network, false, false);
        assert!(matches!(p.decide(&b), LoadVerdict::UntrustedNotApproved));
        let b = bundle("a", BundleSource::Untrusted, SourceLocation::Network, true, false);
        assert!(matches!(p.decide(&b), LoadVerdict::UntrustedNotApproved));
        let b = bundle("a", BundleSource::Untrusted, SourceLocation::Network, true, true);
        assert!(matches!(p.decide(&b), LoadVerdict::Loaded));
    }
    ...
    fn schema_mismatch_rejected() {
        let p = BundleLoadPolicy::new();
        let mut b = bundle("a", BundleSource::Vendor, SourceLocation::Local, true, false);
    ...
    fn policy_schema_drift_rejected() {
        ...
        assert!(matches!(p.validate().unwrap_err(), BundleLoadError::SchemaMismatch));
    }

    #[test]
    fn verdict_serde_kebab() {
        let v = LoadVerdict::UntrustedNotApproved;
        assert!(serde_json::to_string(&v).unwrap().contains("\"kind\":\"untrusted-not-approved\""));
    }
