...
mod tests {
    ...
    fn plain_text_always_pass() {
        ...
        assert!(matches!(
            p.check("t", "anything goes").unwrap(),
            ShapeCheck::Pass
        ));
    }
    ...
    fn markdown_html_doctype_rejected() {
        ...
        assert!(matches!(
            p.check("t", "<!DOCTYPE html><html>").unwrap(),
            ShapeCheck::ShapeMismatch { .. }
        ));
    }
    ...
    fn duplicate_tool_rejected() {
        ...
        p.register("t", OutputShape::Json).unwrap();
        assert!(matches!(
            p.register("t", OutputShape::Yaml).unwrap_err(),
            LangError::DuplicateToolId(_)
        ));
    }
