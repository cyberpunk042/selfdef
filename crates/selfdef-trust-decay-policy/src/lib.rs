...
impl TrustDecayPolicy {
    ...
    pub fn validate(&self) -> Result<(), DecayError> {
        ...
        for r in &self.rules {
            if r.half_life_days == 0 {
                return Err(DecayError::ZeroHalfLife(r.cohort));
            }
            if r.floor_score > 100 {
                return Err(DecayError::FloorOutOfRange(r.cohort));
            }
        }
        Ok(())
    }
    ...
    pub fn apply_decay(
        ...
        current_score: u8,
        elapsed_days: u32,
    ) -> Result<u8, DecayError> {
        if current_score > 100 {
            return Err(DecayError::ScoreOutOfRange(current_score));
        }
        let rule = self.get(cohort).ok_or(DecayError::Missing(cohort))?;
        if elapsed_days == 0 {
            return Ok(current_score);
        }
        if current_score <= rule.floor_score {
            return Ok(current_score);
        }
...
mod tests {
    ...
    fn score_out_of_range_caught() {
        let p = TrustDecayPolicy::canonical();
        assert!(matches!(
            p.apply_decay(Cohort::Trusted, 150, 1).unwrap_err(),
