...
impl PolicyStateSnapshot {
    ...
    pub fn build(engine_version: &str, snapshot_at_unix: u64, policy_blob: &str) -> Result<Self, SnapshotError> {
        if engine_version.is_empty() { return Err(SnapshotError::EmptyEngineVersion); }
        if policy_blob.is_empty() { return Err(SnapshotError::EmptyPolicyBlob); }
    ...
    pub fn validate(&self) -> Result<(), SnapshotError> {
        ...
        if self.engine_version.is_empty() { return Err(SnapshotError::EmptyEngineVersion); }
        if self.policy_blob.is_empty() { return Err(SnapshotError::EmptyPolicyBlob); }
        self.verify_digest()
    }
}
...
fn valid_hex(s: &str) -> bool {
    s.len() == 16 && s.chars().all(|c| c.is_ascii_hexdigit() && (c.is_ascii_digit() || c.is_ascii_lowercase()))
}
...
mod tests {
    ...
    fn tampered_blob_detected() {
        ...
        assert!(matches!(s.verify_digest().unwrap_err(), SnapshotError::DigestMismatch { .. }));
    }
    ...
    fn bad_digest_format_rejected() {
        ...
        assert!(matches!(s.verify_digest().unwrap_err(), SnapshotError::BadDigestFormat));
    }
