...
impl SessionResumePolicy {
    ...
    pub fn new(soft_resume_window_ms: u64, reauth_window_ms: u64) -> Result<Self, ResumeError> {
        if reauth_window_ms < soft_resume_window_ms {
            return Err(ResumeError::BadWindows { soft: soft_resume_window_ms, reauth: reauth_window_ms });
        }
        Ok(Self {
    ...
    /// Decide.
    pub fn decide(&self, disconnected_at_ms: u64, now_ms: u64, cause: DisconnectCause) -> ResumeVerdict {
        ...
        if matches!(cause, DisconnectCause::OperatorLogout) {
            ...
            return ResumeVerdict::RequireNewSession { since_disconnect_ms: since };
        }
        if since <= self.soft_resume_window_ms {
            return ResumeVerdict::ResumeFreely;
        }
        if since <= self.reauth_window_ms {
            return ResumeVerdict::RequireReauth { since_disconnect_ms: since };
        }
        ResumeVerdict::RequireNewSession { since_disconnect_ms: since }
    }

    /// Validate.
    pub fn validate(&self) -> Result<(), ResumeError> {
        if self.schema_version != SCHEMA_VERSION { return Err(ResumeError::SchemaMismatch); }
...
impl Default for SessionResumePolicy {
    fn default() -> Self { Self::new(60_000, 3_600_000).unwrap() }
}
...
mod tests {
    ...
    fn freely_within_soft_window() {
        ...
        assert_eq!(p.decide(0, 500, DisconnectCause::NetworkInterruption), ResumeVerdict::ResumeFreely);
    }
    ...
    fn reauth_in_intermediate_window() {
        ...
        match p.decide(0, 5000, DisconnectCause::NetworkInterruption) {
            ResumeVerdict::RequireReauth { since_disconnect_ms } => assert_eq!(since_disconnect_ms, 5000),
            _ => panic!(),
        }
    }
    ...
    fn new_session_past_reauth() {
        let p = SessionResumePolicy::new(1000, 10_000).unwrap();
        assert!(matches!(p.decide(0, 20_000, DisconnectCause::NetworkInterruption), ResumeVerdict::RequireNewSession { .. }));
    }
    ...
    fn operator_logout_always_new() {
        ...
        assert!(matches!(p.decide(0, 1, DisconnectCause::OperatorLogout), ResumeVerdict::RequireNewSession { .. }));
    }

    #[test]
    fn idle_timeout_uses_normal_windows() {
        let p = SessionResumePolicy::new(1000, 10_000).unwrap();
        assert_eq!(p.decide(0, 500, DisconnectCause::IdleTimeout), ResumeVerdict::ResumeFreely);
        assert!(matches!(p.decide(0, 5000, DisconnectCause::IdleTimeout), ResumeVerdict::RequireReauth { .. }));
    }
    ...
    fn boundary_exact() {
        ...
        assert_eq!(p.decide(0, 1000, DisconnectCause::NetworkInterruption), ResumeVerdict::ResumeFreely);
        // At exactly reauth window.
        assert!(matches!(p.decide(0, 10_000, DisconnectCause::NetworkInterruption), ResumeVerdict::RequireReauth { .. }));
    }

    #[test]
    fn bad_windows_rejected() {
        assert!(matches!(SessionResumePolicy::new(10, 5).unwrap_err(), ResumeError::BadWindows { .. }));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(p.validate().unwrap_err(), ResumeError::SchemaMismatch));
    }
