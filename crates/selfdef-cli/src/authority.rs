...
pub(crate) fn run_matrix() -> Result<i32> {
    ...
    for (c, role) in &[
        ("selfdef-mode-transition-authority",   "227 LOC — 4-gate matrix"),
        ("selfdef-toggle-audit-authority",      "270 LOC — toggle policy"),
        ("selfdef-config-mutation-authority",   "225 LOC — config gating"),
        ("selfdef-recovery-snapshot-authority", "257 LOC — snapshot gate"),
        ("selfdef-profile-authority-gate",      "660 LOC — 6-profile matrix"),
    ] {
        println!("  - {:<35} {}", c, role);
    }
    Ok(0)
