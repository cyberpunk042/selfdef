...
impl QuotaTable {
    ...
    /// Register.
    pub fn register(&mut self, key: &str, limit: u64, period_ms: u64, now_ms: u64) -> Result<(), QuotaError> {
        if key.is_empty() { return Err(QuotaError::EmptyKey); }
        if limit == 0 { return Err(QuotaError::ZeroLimit); }
        if period_ms == 0 { return Err(QuotaError::ZeroPeriod); }
        self.quotas.insert(key.into(), Quota { limit, period_ms, used: 0, last_reset_ms: now_ms });
        Ok(())
    }
    ...
    pub fn consume(&mut self, key: &str, amount: u64, now_ms: u64) -> Result<(), QuotaError> {
        if amount == 0 { return Err(QuotaError::ZeroAmount); }
        if !self.quotas.contains_key(key) { return Err(QuotaError::UnknownKey(key.into())); }
        ...
        if amount > available {
            ...
            return Err(QuotaError::Exceeded { need: amount, available });
        }
    ...
    pub fn remaining(&mut self, key: &str, now_ms: u64) -> Option<u64> {
        if !self.quotas.contains_key(key) { return None; }
        self.advance(key, now_ms);
    ...
    pub fn validate(&self) -> Result<(), QuotaError> {
        ...
        for (k, q) in &self.quotas {
            if k.is_empty() { return Err(QuotaError::EmptyKey); }
            if q.limit == 0 { return Err(QuotaError::ZeroLimit); }
            if q.period_ms == 0 { return Err(QuotaError::ZeroPeriod); }
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn exceeded_rejected() {
        ...
        assert!(matches!(t.consume("api", 5, 200).unwrap_err(), QuotaError::Exceeded { .. }));
    ...
    fn unknown_key_rejected() {
        let mut t = QuotaTable::new();
        assert!(matches!(t.consume("nope", 1, 0).unwrap_err(), QuotaError::UnknownKey(_)));
    }

    #[test]
    fn bad_inputs_rejected() {
        let mut t = QuotaTable::new();
        assert!(matches!(t.register("", 1, 1, 0).unwrap_err(), QuotaError::EmptyKey));
        assert!(matches!(t.register("a", 0, 1, 0).unwrap_err(), QuotaError::ZeroLimit));
        assert!(matches!(t.register("a", 1, 0, 0).unwrap_err(), QuotaError::ZeroPeriod));
        t.register("a", 1, 1, 0).unwrap();
        assert!(matches!(t.consume("a", 0, 0).unwrap_err(), QuotaError::ZeroAmount));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(t.validate().unwrap_err(), QuotaError::SchemaMismatch));
    }
