...
impl SubstrateHeartbeatPolicy {
    ...
    pub fn register(&mut self, component_id: &str, deadline_ms: u64) -> Result<(), HeartbeatError> {
        if component_id.is_empty() { return Err(HeartbeatError::EmptyId); }
    ...
    pub fn beat(&mut self, component_id: &str, ts_ms: u64) -> Result<(), HeartbeatError> {
        if component_id.is_empty() { return Err(HeartbeatError::EmptyId); }
        let entry = self.components.entry(component_id.into()).or_insert(Component { deadline_ms: 60_000, last_beat_ms: None });
        ...
        match e.last_beat_ms {
            ...
            Some(t) => {
                ...
                if age > e.deadline_ms {
                    HeartbeatVerdict::Stale { age_ms: age, deadline_ms: e.deadline_ms }
                } else {
    ...
    pub fn stale_set(&self, now_ms: u64) -> Vec<String> {
        self.components.iter()
            .filter(|(_, e)| matches!(
                e.last_beat_ms.map(|t| now_ms.saturating_sub(t) > e.deadline_ms),
                Some(true)
            ))
            .map(|(k, _)| k.clone())
            .collect()
    ...
    pub fn validate(&self) -> Result<(), HeartbeatError> {
        if self.schema_version != SCHEMA_VERSION { return Err(HeartbeatError::SchemaMismatch); }
        for k in self.components.keys() {
            if k.is_empty() { return Err(HeartbeatError::EmptyId); }
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn stale_past_deadline() {
        ...
        assert_eq!(v, HeartbeatVerdict::Stale { age_ms: 5000, deadline_ms: 1000 });
    }
    ...
    fn beat_preserved_across_re_register() {
        ...
        assert!(matches!(p.check("worker", 200), HeartbeatVerdict::Live { .. }));
    }
    ...
    fn nonmonotonic_rejected() {
        ...
        p.beat("w", 200).unwrap();
        assert!(matches!(p.beat("w", 100).unwrap_err(), HeartbeatError::NonMonotonic { .. }));
    }

    #[test]
    fn empty_id_rejected() {
        let mut p = SubstrateHeartbeatPolicy::new();
        assert!(matches!(p.register("", 1000).unwrap_err(), HeartbeatError::EmptyId));
        assert!(matches!(p.beat("", 0).unwrap_err(), HeartbeatError::EmptyId));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(p.validate().unwrap_err(), HeartbeatError::SchemaMismatch));
    }
