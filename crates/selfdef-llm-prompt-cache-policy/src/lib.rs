...
impl LlmPromptCachePolicy {
    ...
    pub fn classify(&self, profile: Profile, kind: PromptKind, contains_secret: bool) -> Verdict {
        if contains_secret {
            return Verdict::NoCache {
                reason: NoCacheReason::SecretLeakRisk,
            };
        }
        let by_kind = match self.decisions.get(&profile) {
            ...
            None => {
                return Verdict::NoCache {
                    reason: NoCacheReason::Unconfigured,
                };
            }
        };
...
mod tests {
    ...
    fn user_turn_no_cache_in_production() {
        ...
        assert_eq!(
            ...
            Verdict::NoCache {
                reason: NoCacheReason::PolicyDeny
            }
        );
    }
    ...
    fn private_only_pinned_caches() {
        let p = LlmPromptCachePolicy::canonical();
        assert!(matches!(
            p.classify(Profile::Private, PromptKind::Pinned, false),
            Verdict::Cache { .. }
        ));
        for k in [
            PromptKind::System,
            PromptKind::UserTurn,
            PromptKind::ToolOutput,
            PromptKind::Scratch,
        ] {
            assert!(matches!(
                p.classify(Profile::Private, k, false),
                Verdict::NoCache { .. }
            ));
        }
    }
    ...
    fn experimental_caches_scratch() {
        let p = LlmPromptCachePolicy::canonical();
        assert!(matches!(
            p.classify(Profile::Experimental, PromptKind::Scratch, false),
            Verdict::Cache { .. }
        ));
    }
    ...
    fn unconfigured_profile() {
        ...
        p.decisions.clear();
        assert_eq!(
            p.classify(Profile::Fast, PromptKind::System, false),
            Verdict::NoCache {
                reason: NoCacheReason::Unconfigured
            }
        );
