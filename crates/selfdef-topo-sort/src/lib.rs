...
impl TopoSort {
    ...
    pub fn add_node(&mut self, id: &str) -> Result<(), TopoError> {
        if id.is_empty() { return Err(TopoError::EmptyNode); }
        if self.nodes.contains_key(id) { return Err(TopoError::DuplicateNode(id.into())); }
    ...
    pub fn add_edge(&mut self, from: &str, to: &str) -> Result<(), TopoError> {
        if from == to { return Err(TopoError::SelfEdge(from.into())); }
        if !self.nodes.contains_key(from) { return Err(TopoError::UnknownNode(from.into())); }
        if !self.nodes.contains_key(to) { return Err(TopoError::UnknownNode(to.into())); }
    ...
    pub fn sort(&self) -> Result<Vec<String>, TopoError> {
        // Indegree.
        let mut indegree: BTreeMap<String, u32> = self.nodes.keys().map(|k| (k.clone(), 0)).collect();
    ...
    pub fn validate(&self) -> Result<(), TopoError> {
        if self.schema_version != SCHEMA_VERSION { return Err(TopoError::SchemaMismatch); }
        for (k, outs) in &self.nodes {
            if k.is_empty() { return Err(TopoError::EmptyNode); }
            for v in outs {
                if !self.nodes.contains_key(v) { return Err(TopoError::UnknownNode(v.clone())); }
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn diamond() {
        let mut t = TopoSort::new();
        for n in &["a", "b", "c", "d"] { t.add_node(n).unwrap(); }
        t.add_edge("a", "b").unwrap();
    ...
    fn self_edge_rejected() {
        let mut t = TopoSort::new();
        t.add_node("a").unwrap();
        assert!(matches!(t.add_edge("a", "a").unwrap_err(), TopoError::SelfEdge(_)));
    }
    ...
    fn unknown_endpoint_rejected() {
        let mut t = TopoSort::new();
        t.add_node("a").unwrap();
        assert!(matches!(t.add_edge("a", "b").unwrap_err(), TopoError::UnknownNode(_)));
    }
    ...
    fn deterministic_ties() {
        ...
        let mut t = TopoSort::new();
        for n in &["b", "a", "c"] { t.add_node(n).unwrap(); }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(t.validate().unwrap_err(), TopoError::SchemaMismatch));
    }
