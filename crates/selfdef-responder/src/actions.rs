...
impl Action for RevokeSessionAction {
    ...
    async fn execute(&self, event: &Event, dry_run: bool) -> Result<ActionOutcome, ActionError> {
        ...
        // `--` separator, so a value like `-oProxyCommand=…` or `--config=…`
...
mod tests {
    ...
    async fn revoke_session_refuses_username_with_leading_dash() {
        // Argv-injection guard: event.actor.user.name is attacker-influenceable
        // (auditd `acct=` field), and the script-based RevokeSessionAction runs
        // `script <user>` with no `--` separator. A username starting with `-`
        // would be flag-parsed by the script (e.g. `-oProxyCommand=…`). The
        // action must refuse it fail-closed BEFORE invoking the script —
        // verified in dry-run so no subprocess is spawned. Without the guard
        // this returns DryRun ("would invoke … -oProxyCommand=evil"); with it,
        // Skipped.
        ...
        assert!(
            ...
            "refusal note should explain why: {}",
            outcome.notes
        );

        // A normal username still previews a real invocation (regression guard).
        let ok = action
