...
impl GrantTemplateAllowlist {
    ...
    pub fn canonical() -> Self {
        ...
        p.insert(
            Profile::Careful,
            ["read-local".into(), "watch-fs".into(), "write-tmp".into()]
                .into_iter()
                .collect(),
        );
        p.insert(
    ...
    pub fn classify(
        ...
        profile: Profile,
        template_id: &str,
    ) -> Result<AllowVerdict, AllowError> {
        if template_id.is_empty() {
            return Err(AllowError::EmptyId);
        }
...
mod tests {
    ...
    fn allowed_in_profile() {
        ...
        assert_eq!(
            a.classify(Profile::Production, "read-local").unwrap(),
            AllowVerdict::Allowed
        );
    }
    ...
    fn wildcard_admits_all() {
        let a = GrantTemplateAllowlist::canonical();
        assert_eq!(
            a.classify(Profile::Experimental, "weird-new-grant")
                .unwrap(),
            AllowVerdict::Allowed
        );
    }
    ...
    fn autonomous_has_more_than_production() {
        ...
        assert_eq!(
            a.classify(Profile::Autonomous, "spawn-child").unwrap(),
            AllowVerdict::Allowed
        );
        assert!(matches!(
            a.classify(Profile::Production, "spawn-child").unwrap(),
            AllowVerdict::Denied { .. }
        ));
    }
