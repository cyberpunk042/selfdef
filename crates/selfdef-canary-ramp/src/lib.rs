...
impl CanaryRamp {
    ...
    pub fn new(points: Vec<Point>, seed: u64) -> Result<Self, RampError> {
        if seed == 0 {
            return Err(RampError::ZeroSeed);
        }
        for p in &points {
            if p.percent_bp > 10_000 {
                return Err(RampError::BadPercent);
            }
        }
        for w in points.windows(2) {
            if w[0].ts_ms >= w[1].ts_ms {
                return Err(RampError::NotStrictlyIncreasing);
            }
        }
        Ok(Self {
...
mod tests {
    ...
    fn percent_0_denies_all() {
        ...
        let mut admitted = 0;
        for k in 0..200 {
            if r.admit(0, &k.to_string()).unwrap() {
                admitted += 1;
            }
        }
    ...
    fn percent_100_admits_all() {
        ...
        let mut admitted = 0;
        for k in 0..200 {
            if r.admit(2000, &k.to_string()).unwrap() {
                admitted += 1;
            }
        }
    ...
    fn percent_50_admits_about_half() {
        ...
        let mut admitted = 0;
        for k in 0..2000 {
            if r.admit(1000, &k.to_string()).unwrap() {
                admitted += 1;
            }
        }
        // Expect ~1000 ± 15%.
    ...
    fn deterministic_same_inputs() {
        ...
        for k in 0..50 {
            assert_eq!(
                a.admit(1000, &k.to_string()).unwrap(),
                b.admit(1000, &k.to_string()).unwrap()
            );
        }
    }
    ...
    fn bad_inputs_rejected() {
        ...
        assert!(matches!(
            CanaryRamp::new(vec![], 0).unwrap_err(),
            RampError::ZeroSeed
        ));
        let mut r = ramp();
