...
fn atomic_write(path: &Path, body: &str) -> Result<(), (StatusCode, String)> {
    ...
    // after a profile write cannot lose the mutation, resurrect a stale file,
    // or leave a zero-length profile the next read falls back from. The rename
    // alone gives crash consistency (no torn read) but not durability. fsync
    // the parent directory afterward so the rename's new directory entry is
