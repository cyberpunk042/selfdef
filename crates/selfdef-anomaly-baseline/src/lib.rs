...
impl AnomalyBaseline {
    /// New.
    pub fn new(window_size: u32, min_samples: u32, suspect_z: f64, anomalous_z: f64) -> Result<Self, BaselineError> {
        if window_size == 0 { return Err(BaselineError::WindowZero); }
        if min_samples > window_size { return Err(BaselineError::MinExceedsWindow(min_samples, window_size)); }
        if anomalous_z < suspect_z { return Err(BaselineError::BadThresholds { suspect_z, anomalous_z }); }
        Ok(Self {
    ...
    pub fn observe(&mut self, value: f64) -> Result<Observation, BaselineError> {
        if value.is_nan() { return Err(BaselineError::NanValue); }
    ...
    pub fn validate(&self) -> Result<(), BaselineError> {
        ...
        if self.window_size == 0 { return Err(BaselineError::WindowZero); }
        if self.min_samples > self.window_size {
            return Err(BaselineError::MinExceedsWindow(self.min_samples, self.window_size));
        }
...
mod tests {
    ...
    fn zero_window_rejected() {
        assert!(matches!(AnomalyBaseline::new(0, 0, 2.0, 4.0).unwrap_err(), BaselineError::WindowZero));
    }
    ...
    fn stable_samples_stay_normal() {
        ...
        for _ in 0..15 { b.observe(10.0).unwrap(); }
    ...
    fn nan_value_rejected() {
        ...
        assert!(matches!(b.observe(f64::NAN).unwrap_err(), BaselineError::NanValue));
    }
    ...
    fn schema_drift_rejected() {
        ...
        assert!(matches!(b.validate().unwrap_err(), BaselineError::SchemaMismatch));
    }

    #[test]
    fn tier_serde_kebab() {
        assert_eq!(serde_json::to_string(&AnomalyTier::Anomalous).unwrap(), "\"anomalous\"");
    }
