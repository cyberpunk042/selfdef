...
| Notification credentials | `/etc/selfdef/secrets/` | ntfy tokens, signal-cli auth, SMTP `password_file` (`[notifier.smtp]` per SDD-008 D-7 Q-E), Twilio `auth_token_file` (`[notifier.twilio]` per SDD-008 Q-D), Slack `webhook_url_file` (`[notifier.slack]` per SDD-008 Q-C), Discord `webhook_url_file` (`[notifier.discord]` per SDD-008) — both webhook URLs are themselves the auth secret. Each integration crate reads its credential from a disk path; daemon must run as the file owner and the file mode must keep group/world readers out (`0600` recommended). |
| TTY broadcast | `wall(1)` via `[notifier.wall]` (SDD-008 D-8) | The wall session-attention channel writes one-line attention banners to every logged-in TTY when severity ≥ floor (default `high`). Daemon needs permission to wall (typically `tty`-group membership or root). Defense-in-depth: severity_floor refuses to broadcast on routine events; operators who lower it past the default carry the noise themselves. |
