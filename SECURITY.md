...
| Notification credentials | `/etc/selfdef/secrets/` | ntfy tokens, signal-cli auth, SMTP `password_file` (`[notifier.smtp]` per SDD-008 D-7 Q-E), Twilio `auth_token_file` (`[notifier.twilio]` per SDD-008 Q-D). Each integration crate reads its credential from a disk path; daemon must run as the file owner and the file mode must keep group/world readers out (`0600` recommended). |
