...
or:  ``python3 -m unittest tests.observability.test_apply_sh_end_to_end_integration``

Stdlib-unittest shaped (pytest collects unittest.TestCase natively) so the
suite runs on a pytest-less box too — same portability bar as coherence.sh's
"works on a bats-only box". The previous module-scoped pytest fixture made
``unittest discover`` fail at IMPORT time on any box without pytest,
silently excluding all 9 of these integration locks from the stdlib run.
"""
...
class TestApplyShEndToEnd(unittest.TestCase):
    """Deployment-shape locks against ONE shared apply.sh run (the
    former module-scoped fixture, now class-scoped setUpClass)."""
    ...
    def setUpClass(cls) -> None:
        cls.scrape, cls.dashboard, cls.alerts = _run_apply()
        # Cleanup is best-effort — leave it for tempdir cleanup.

    def test_apply_sh_exits_clean_under_default_config(self):
        assert self.scrape.is_file(), f"scrape config not deployed: {self.scrape}"
        assert self.dashboard.is_file(), f"dashboard not deployed: {self.dashboard}"
        assert self.alerts.is_file(), f"alerts not deployed: {self.alerts}"

    def test_deployed_scrape_yaml_is_valid(self):
        doc = yaml.safe_load(self.scrape.read_text())
        assert doc is not None, "scrape YAML failed to load"
        assert "scrape_configs" in doc
        assert isinstance(doc["scrape_configs"], list)

    def test_deployed_scrape_has_both_jobs(self):
    ...
    def test_deployed_daemon_job_carries_bearer_token(self):
    ...
    def test_deployed_targets_routed_by_port(self):
    ...
    def test_deployed_dashboard_is_valid_json(self):
        doc = json.loads(self.dashboard.read_text())
        assert "panels" in doc
        assert isinstance(doc["panels"], list)
        assert len(doc["panels"]) > 0

    def test_deployed_dashboard_has_substituted_title_and_uid(self):
        raw = self.dashboard.read_text()
    ...
    def test_deployed_alerts_yaml_is_valid_and_complete(self):
        ...
        doc = yaml.safe_load(self.alerts.read_text())
...
class TestApplyShIdempotency(unittest.TestCase):
    def test_idempotent_re_apply_makes_no_changes(self):
