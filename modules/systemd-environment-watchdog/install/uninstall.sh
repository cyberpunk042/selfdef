#!/usr/bin/env bash
# systemd-environment-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-systemd-env.timer || true
    systemctl reset-failed selfdef-systemd-env.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-systemd-env.service" \
         "${SYSTEMD_DIR}/selfdef-systemd-env.timer" \
         "${LIBEXEC_DIR}/systemd-environment-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "systemd-environment-watchdog removed=$removed (baseline preserved)"
