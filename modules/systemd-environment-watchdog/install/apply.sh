#!/usr/bin/env bash
# systemd-environment-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/systemd-environment-watchdog.sh" "${LIBEXEC_DIR}/systemd-environment-watchdog.sh" "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-systemd-env.service"     "${SYSTEMD_DIR}/selfdef-systemd-env.service"     "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-systemd-env.timer"       "${SYSTEMD_DIR}/selfdef-systemd-env.timer"       "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-systemd-env.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SYSTEMDENV_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-systemd-env.timer" -- \
        systemctl enable --now selfdef-systemd-env.timer || true
fi

emit_status "ok" "systemd-environment-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
