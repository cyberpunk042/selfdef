#!/usr/bin/env bash
# ssh-client-config-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/ssh-client-config-watchdog.sh"     "${LIBEXEC_DIR}/ssh-client-config-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-client-config.service" "${SYSTEMD_DIR}/selfdef-ssh-client-config.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-client-config.timer"   "${SYSTEMD_DIR}/selfdef-ssh-client-config.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-ssh-client-config.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SSHCLIENT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-ssh-client-config.timer" -- \
        systemctl enable --now selfdef-ssh-client-config.timer || true
fi

emit_status "ok" "ssh-client-config-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
