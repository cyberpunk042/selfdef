...
changes=0
install_one "${SYSTEMD_SRC}/ld-preload-watchdog.sh"        "${LIBEXEC_DIR}/ld-preload-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ld-preload.service"    "${SYSTEMD_DIR}/selfdef-ld-preload.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ld-preload.timer"      "${SYSTEMD_DIR}/selfdef-ld-preload.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-ld-preload.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_LDPRELOAD_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-ld-preload.timer" -- \
        systemctl enable --now selfdef-ld-preload.timer || true
fi

emit_status "ok" "ld-preload-watchdog profile=$PROFILE changes=$changes"
