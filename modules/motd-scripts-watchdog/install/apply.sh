...
changes=0
install_one "${SYSTEMD_SRC}/motd-scripts-watchdog.sh"     "${LIBEXEC_DIR}/motd-scripts-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-motd-scripts.service" "${SYSTEMD_DIR}/selfdef-motd-scripts.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-motd-scripts.timer"   "${SYSTEMD_DIR}/selfdef-motd-scripts.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-motd-scripts.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_MOTD_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-motd-scripts.timer" -- \
        systemctl enable --now selfdef-motd-scripts.timer || true
fi

emit_status "ok" "motd-scripts-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
