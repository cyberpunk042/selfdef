#!/usr/bin/env bash
# logfile-integrity-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/logfile-integrity-watchdog.sh"        "${LIBEXEC_DIR}/logfile-integrity-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-logfile-integrity.service"    "${SYSTEMD_DIR}/selfdef-logfile-integrity.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-logfile-integrity.timer"      "${SYSTEMD_DIR}/selfdef-logfile-integrity.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-logfile-integrity.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_LOGINT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-logfile-integrity.timer" -- \
        systemctl enable --now selfdef-logfile-integrity.timer || true
fi

emit_status "ok" "logfile-integrity-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
