#!/usr/bin/env bash
# listening-ports-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/listening-ports-watchdog.sh"        "${LIBEXEC_DIR}/listening-ports-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-listening-ports.service"    "${SYSTEMD_DIR}/selfdef-listening-ports.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-listening-ports.timer"      "${SYSTEMD_DIR}/selfdef-listening-ports.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-listening-ports.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_LISTENPORTS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-listening-ports.timer" -- \
        systemctl enable --now selfdef-listening-ports.timer || true
fi

emit_status "ok" "listening-ports-watchdog profile=$PROFILE changes=$changes (first scan will baseline)"
