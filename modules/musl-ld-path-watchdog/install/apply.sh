...
changes=0
install_one "${SYSTEMD_SRC}/musl-ld-path-watchdog.sh"     "${LIBEXEC_DIR}/musl-ld-path-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-musl-ld-path.service" "${SYSTEMD_DIR}/selfdef-musl-ld-path.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-musl-ld-path.timer"   "${SYSTEMD_DIR}/selfdef-musl-ld-path.timer"   "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_MUSL_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-musl-ld-path.timer" -- \
        systemctl enable --now selfdef-musl-ld-path.timer || true
fi

emit_status "ok" "musl-ld-path-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
