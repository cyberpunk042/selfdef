...
changes=0
install_one "${SYSTEMD_SRC}/dhclient-hooks-watchdog.sh"     "${LIBEXEC_DIR}/dhclient-hooks-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-dhclient-hooks.service" "${SYSTEMD_DIR}/selfdef-dhclient-hooks.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-dhclient-hooks.timer"   "${SYSTEMD_DIR}/selfdef-dhclient-hooks.timer"   "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_DHCLIENT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-dhclient-hooks.timer" -- \
        systemctl enable --now selfdef-dhclient-hooks.timer || true
fi

emit_status "ok" "dhclient-hooks-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
