...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-fstab.timer || true
    systemctl reset-failed selfdef-fstab.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-fstab.service" \
         "${SYSTEMD_DIR}/selfdef-fstab.timer" \
         "${LIBEXEC_DIR}/fstab-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "fstab-watchdog removed=$removed (baseline preserved)"
