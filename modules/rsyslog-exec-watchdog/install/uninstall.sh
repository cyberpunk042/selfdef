#!/usr/bin/env bash
# rsyslog-exec-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-rsyslog-exec.timer || true
    systemctl reset-failed selfdef-rsyslog-exec.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-rsyslog-exec.service" \
         "${SYSTEMD_DIR}/selfdef-rsyslog-exec.timer" \
         "${LIBEXEC_DIR}/rsyslog-exec-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "rsyslog-exec-watchdog removed=$removed (baseline preserved)"
