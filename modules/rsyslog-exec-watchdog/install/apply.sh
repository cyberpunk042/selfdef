...
changes=0
install_one "${SYSTEMD_SRC}/rsyslog-exec-watchdog.sh"     "${LIBEXEC_DIR}/rsyslog-exec-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-rsyslog-exec.service" "${SYSTEMD_DIR}/selfdef-rsyslog-exec.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-rsyslog-exec.timer"   "${SYSTEMD_DIR}/selfdef-rsyslog-exec.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-rsyslog-exec.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_RSYSLOG_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-rsyslog-exec.timer" -- \
        systemctl enable --now selfdef-rsyslog-exec.timer || true
fi

emit_status "ok" "rsyslog-exec-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
