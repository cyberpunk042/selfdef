...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-pci-device.timer || true
    systemctl reset-failed selfdef-pci-device.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-pci-device.service" \
         "${SYSTEMD_DIR}/selfdef-pci-device.timer" \
         "${LIBEXEC_DIR}/pci-device-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "pci-device-watchdog removed=$removed (baseline preserved)"
