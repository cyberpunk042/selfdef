...
changes=0
install_one "${SYSTEMD_SRC}/pci-device-watchdog.sh"        "${LIBEXEC_DIR}/pci-device-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-pci-device.service"    "${SYSTEMD_DIR}/selfdef-pci-device.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-pci-device.timer"      "${SYSTEMD_DIR}/selfdef-pci-device.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-pci-device.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_PCIDEV_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-pci-device.timer" -- \
        systemctl enable --now selfdef-pci-device.timer || true
fi

emit_status "ok" "pci-device-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
