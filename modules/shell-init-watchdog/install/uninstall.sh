...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-shell-init.timer || true
    systemctl reset-failed selfdef-shell-init.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-shell-init.service" \
         "${SYSTEMD_DIR}/selfdef-shell-init.timer" \
         "${LIBEXEC_DIR}/shell-init-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "shell-init-watchdog removed=$removed (baseline preserved)"
