...
changes=0
install_one "${SYSTEMD_SRC}/shell-init-watchdog.sh"     "${LIBEXEC_DIR}/shell-init-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-shell-init.service" "${SYSTEMD_DIR}/selfdef-shell-init.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-shell-init.timer"   "${SYSTEMD_DIR}/selfdef-shell-init.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-shell-init.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SHELLINIT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-shell-init.timer" -- \
        systemctl enable --now selfdef-shell-init.timer || true
fi

emit_status "ok" "shell-init-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
