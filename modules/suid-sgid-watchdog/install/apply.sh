...
changes=0
install_one "${SYSTEMD_SRC}/suid-sgid-watchdog.sh"        "${LIBEXEC_DIR}/suid-sgid-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-suid-sgid.service"    "${SYSTEMD_DIR}/selfdef-suid-sgid.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-suid-sgid.timer"      "${SYSTEMD_DIR}/selfdef-suid-sgid.timer"      "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SUIDSGID_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-suid-sgid.timer" -- \
        systemctl enable --now selfdef-suid-sgid.timer || true
fi

emit_status "ok" "suid-sgid-watchdog profile=$PROFILE changes=$changes (first scan will baseline)"
