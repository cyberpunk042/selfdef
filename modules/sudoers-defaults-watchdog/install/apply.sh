#!/usr/bin/env bash
# sudoers-defaults-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/sudoers-defaults-watchdog.sh"     "${LIBEXEC_DIR}/sudoers-defaults-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sudoers-defaults.service" "${SYSTEMD_DIR}/selfdef-sudoers-defaults.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sudoers-defaults.timer"   "${SYSTEMD_DIR}/selfdef-sudoers-defaults.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-sudoers-defaults.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SUDODEF_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-sudoers-defaults.timer" -- \
        systemctl enable --now selfdef-sudoers-defaults.timer || true
fi

emit_status "ok" "sudoers-defaults-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
