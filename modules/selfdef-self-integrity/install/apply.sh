...
changes=0
install_one "${SYSTEMD_SRC}/selfdef-self-integrity.sh"        "${LIBEXEC_DIR}/selfdef-self-integrity.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-self-integrity.service"   "${SYSTEMD_DIR}/selfdef-self-integrity.service"   "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-self-integrity.timer"     "${SYSTEMD_DIR}/selfdef-self-integrity.timer"     "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-self-integrity.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SELFINT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-self-integrity.timer" -- \
        systemctl enable --now selfdef-self-integrity.timer || true
fi

emit_status "ok" "selfdef-self-integrity profile=$PROFILE changes=$changes (first scan builds manifest)"
