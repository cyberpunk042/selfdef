...
changes=0
install_one "${SYSTEMD_SRC}/kernel-module-watchdog.sh"        "${LIBEXEC_DIR}/kernel-module-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-kernel-modules.service"   "${SYSTEMD_DIR}/selfdef-kernel-modules.service"   "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-kernel-modules.timer"     "${SYSTEMD_DIR}/selfdef-kernel-modules.timer"     "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-kernel-modules.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_KMOD_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-kernel-modules.timer" -- \
        systemctl enable --now selfdef-kernel-modules.timer || true
fi

emit_status "ok" "kernel-module-watchdog profile=$PROFILE changes=$changes (first scan will baseline)"
