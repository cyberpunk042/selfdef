...
changes=0
install_one "${SYSTEMD_SRC}/ssh-hostkey-watchdog.sh"        "${LIBEXEC_DIR}/ssh-hostkey-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-hostkey.service"    "${SYSTEMD_DIR}/selfdef-ssh-hostkey.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-hostkey.timer"      "${SYSTEMD_DIR}/selfdef-ssh-hostkey.timer"      "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_HOSTKEY_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-ssh-hostkey.timer" -- \
        systemctl enable --now selfdef-ssh-hostkey.timer || true
fi

emit_status "ok" "ssh-hostkey-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
