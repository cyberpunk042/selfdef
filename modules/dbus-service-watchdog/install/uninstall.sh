#!/usr/bin/env bash
# dbus-service-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-dbus-service.timer || true
    systemctl reset-failed selfdef-dbus-service.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-dbus-service.service" \
         "${SYSTEMD_DIR}/selfdef-dbus-service.timer" \
         "${LIBEXEC_DIR}/dbus-service-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "dbus-service-watchdog removed=$removed (baseline preserved)"
