...
changes=0
install_one "${SYSTEMD_SRC}/syslog-ng-exec-watchdog.sh"     "${LIBEXEC_DIR}/syslog-ng-exec-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-syslog-ng-exec.service" "${SYSTEMD_DIR}/selfdef-syslog-ng-exec.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-syslog-ng-exec.timer"   "${SYSTEMD_DIR}/selfdef-syslog-ng-exec.timer"   "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SYSLOGNG_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-syslog-ng-exec.timer" -- \
        systemctl enable --now selfdef-syslog-ng-exec.timer || true
fi

emit_status "ok" "syslog-ng-exec-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
