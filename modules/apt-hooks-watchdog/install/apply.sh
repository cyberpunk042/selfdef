...
changes=0
install_one "${SYSTEMD_SRC}/apt-hooks-watchdog.sh"     "${LIBEXEC_DIR}/apt-hooks-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-apt-hooks.service" "${SYSTEMD_DIR}/selfdef-apt-hooks.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-apt-hooks.timer"   "${SYSTEMD_DIR}/selfdef-apt-hooks.timer"   "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_APTHOOK_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-apt-hooks.timer" -- \
        systemctl enable --now selfdef-apt-hooks.timer || true
fi

emit_status "ok" "apt-hooks-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
