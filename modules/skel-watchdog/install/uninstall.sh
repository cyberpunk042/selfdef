...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-skel.timer || true
    systemctl reset-failed selfdef-skel.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-skel.service" \
         "${SYSTEMD_DIR}/selfdef-skel.timer" \
         "${LIBEXEC_DIR}/skel-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "skel-watchdog removed=$removed (baseline preserved)"
