...
changes=0
install_one "${SYSTEMD_SRC}/mta-loopback-detect.sh"        "${LIBEXEC_DIR}/mta-loopback-detect.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-mta-loopback.service"  "${SYSTEMD_DIR}/selfdef-mta-loopback.service"  "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-mta-loopback.timer"    "${SYSTEMD_DIR}/selfdef-mta-loopback.timer"    "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_MTA_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-mta-loopback.timer" -- \
        systemctl enable --now selfdef-mta-loopback.timer || true
fi

emit_status "ok" "mta-loopback-detect profile=$PROFILE changes=$changes"
