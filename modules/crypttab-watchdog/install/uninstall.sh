...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-crypttab.timer || true
    systemctl reset-failed selfdef-crypttab.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-crypttab.service" \
         "${SYSTEMD_DIR}/selfdef-crypttab.timer" \
         "${LIBEXEC_DIR}/crypttab-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "crypttab-watchdog removed=$removed (baseline preserved)"
