...
changes=0
install_one "${SYSTEMD_SRC}/crypttab-watchdog.sh"     "${LIBEXEC_DIR}/crypttab-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-crypttab.service" "${SYSTEMD_DIR}/selfdef-crypttab.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-crypttab.timer"   "${SYSTEMD_DIR}/selfdef-crypttab.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-crypttab.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_CRYPTTAB_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-crypttab.timer" -- \
        systemctl enable --now selfdef-crypttab.timer || true
fi

emit_status "ok" "crypttab-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
