...
changes=0
install_one "${SYSTEMD_SRC}/sshd-config-watchdog.sh"     "${LIBEXEC_DIR}/sshd-config-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sshd-config.service" "${SYSTEMD_DIR}/selfdef-sshd-config.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sshd-config.timer"   "${SYSTEMD_DIR}/selfdef-sshd-config.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-sshd-config.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SSHD_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-sshd-config.timer" -- \
        systemctl enable --now selfdef-sshd-config.timer || true
fi

emit_status "ok" "sshd-config-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
