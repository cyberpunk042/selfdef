#!/usr/bin/env bash
# sshd-config-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-sshd-config.timer || true
    systemctl reset-failed selfdef-sshd-config.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-sshd-config.service" \
         "${SYSTEMD_DIR}/selfdef-sshd-config.timer" \
         "${LIBEXEC_DIR}/sshd-config-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "sshd-config-watchdog removed=$removed (baseline preserved)"
