#!/usr/bin/env bash
# sudoers-integrity-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/sudoers-integrity-watchdog.sh"        "${LIBEXEC_DIR}/sudoers-integrity-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sudoers-integrity.service"    "${SYSTEMD_DIR}/selfdef-sudoers-integrity.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-sudoers-integrity.timer"      "${SYSTEMD_DIR}/selfdef-sudoers-integrity.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-sudoers-integrity.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_SUDOERS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-sudoers-integrity.timer" -- \
        systemctl enable --now selfdef-sudoers-integrity.timer || true
fi

emit_status "ok" "sudoers-integrity-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
