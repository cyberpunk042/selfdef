#!/usr/bin/env bash
# capability-conf-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/capability-conf-watchdog.sh"     "${LIBEXEC_DIR}/capability-conf-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-capability-conf.service" "${SYSTEMD_DIR}/selfdef-capability-conf.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-capability-conf.timer"   "${SYSTEMD_DIR}/selfdef-capability-conf.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-capability-conf.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_CAPCONF_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-capability-conf.timer" -- \
        systemctl enable --now selfdef-capability-conf.timer || true
fi

emit_status "ok" "capability-conf-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
