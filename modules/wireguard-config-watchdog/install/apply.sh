#!/usr/bin/env bash
# wireguard-config-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/wireguard-config-watchdog.sh" "${LIBEXEC_DIR}/wireguard-config-watchdog.sh" "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-wireguard.service"    "${SYSTEMD_DIR}/selfdef-wireguard.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-wireguard.timer"      "${SYSTEMD_DIR}/selfdef-wireguard.timer"      "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_WIREGUARD_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-wireguard.timer" -- \
        systemctl enable --now selfdef-wireguard.timer || true
fi

emit_status "ok" "wireguard-config-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
