...
changes=0
install_one "${SYSTEMD_SRC}/nfs-mount-watchdog.sh"        "${LIBEXEC_DIR}/nfs-mount-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-nfs-mount.service"    "${SYSTEMD_DIR}/selfdef-nfs-mount.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-nfs-mount.timer"      "${SYSTEMD_DIR}/selfdef-nfs-mount.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-nfs-mount.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_NFSMOUNT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-nfs-mount.timer" -- \
        systemctl enable --now selfdef-nfs-mount.timer || true
fi

emit_status "ok" "nfs-mount-watchdog profile=$PROFILE changes=$changes"
