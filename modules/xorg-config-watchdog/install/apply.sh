...
changes=0
install_one "${SYSTEMD_SRC}/xorg-config-watchdog.sh"     "${LIBEXEC_DIR}/xorg-config-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-xorg-config.service" "${SYSTEMD_DIR}/selfdef-xorg-config.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-xorg-config.timer"   "${SYSTEMD_DIR}/selfdef-xorg-config.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-xorg-config.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_XORG_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-xorg-config.timer" -- \
        systemctl enable --now selfdef-xorg-config.timer || true
fi

emit_status "ok" "xorg-config-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
