#!/usr/bin/env bash
# xorg-config-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-xorg-config.timer || true
    systemctl reset-failed selfdef-xorg-config.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-xorg-config.service" \
         "${SYSTEMD_DIR}/selfdef-xorg-config.timer" \
         "${LIBEXEC_DIR}/xorg-config-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "xorg-config-watchdog removed=$removed (baseline preserved)"
