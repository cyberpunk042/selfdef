#!/usr/bin/env bash
# fail2ban-action-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/fail2ban-action-watchdog.sh"     "${LIBEXEC_DIR}/fail2ban-action-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-fail2ban-action.service" "${SYSTEMD_DIR}/selfdef-fail2ban-action.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-fail2ban-action.timer"   "${SYSTEMD_DIR}/selfdef-fail2ban-action.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-fail2ban-action.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_F2BACTION_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-fail2ban-action.timer" -- \
        systemctl enable --now selfdef-fail2ban-action.timer || true
fi

emit_status "ok" "fail2ban-action-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
