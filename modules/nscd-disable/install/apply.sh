...
if [[ "$acted" -eq 0 ]]; then
    log "nscd not installed on this host — no-op"
    emit_status "ok" "nscd-disable no-op (nscd not present)"
    exit 0
fi

emit_status "ok" "nscd-disable profile=$PROFILE acted=$acted skipped=$skipped"
