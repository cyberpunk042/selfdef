...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-gss-mech.timer || true
    systemctl reset-failed selfdef-gss-mech.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-gss-mech.service" \
         "${SYSTEMD_DIR}/selfdef-gss-mech.timer" \
         "${LIBEXEC_DIR}/gss-mech-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "gss-mech-watchdog removed=$removed (baseline preserved)"
