...
changes=0
install_one "${SYSTEMD_SRC}/gss-mech-watchdog.sh"     "${LIBEXEC_DIR}/gss-mech-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-gss-mech.service" "${SYSTEMD_DIR}/selfdef-gss-mech.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-gss-mech.timer"   "${SYSTEMD_DIR}/selfdef-gss-mech.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-gss-mech.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_GSS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-gss-mech.timer" -- \
        systemctl enable --now selfdef-gss-mech.timer || true
fi

emit_status "ok" "gss-mech-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
