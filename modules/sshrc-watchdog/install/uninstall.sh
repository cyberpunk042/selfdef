...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-sshrc.timer || true
    systemctl reset-failed selfdef-sshrc.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-sshrc.service" \
         "${SYSTEMD_DIR}/selfdef-sshrc.timer" \
         "${LIBEXEC_DIR}/sshrc-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "sshrc-watchdog removed=$removed (baseline preserved)"
