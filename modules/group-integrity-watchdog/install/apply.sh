#!/usr/bin/env bash
# group-integrity-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/group-integrity-watchdog.sh"        "${LIBEXEC_DIR}/group-integrity-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-group-integrity.service"    "${SYSTEMD_DIR}/selfdef-group-integrity.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-group-integrity.timer"      "${SYSTEMD_DIR}/selfdef-group-integrity.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-group-integrity.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_GROUPINT_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-group-integrity.timer" -- \
        systemctl enable --now selfdef-group-integrity.timer || true
fi

emit_status "ok" "group-integrity-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
