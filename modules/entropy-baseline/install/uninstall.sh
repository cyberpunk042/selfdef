...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-entropy.timer || true
    systemctl reset-failed selfdef-entropy.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-entropy.service" \
         "${SYSTEMD_DIR}/selfdef-entropy.timer" \
         "${LIBEXEC_DIR}/entropy-baseline.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "entropy-baseline removed=$removed"
