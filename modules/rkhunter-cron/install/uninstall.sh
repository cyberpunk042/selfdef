...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-rkhunter-check.timer || true
    systemctl reset-failed selfdef-rkhunter-check.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-rkhunter-check.service" \
         "${SYSTEMD_DIR}/selfdef-rkhunter-check.timer" \
         "${LIBEXEC_DIR}/rkhunter-check.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "rkhunter-cron removed=$removed"
