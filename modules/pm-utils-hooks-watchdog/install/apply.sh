...
changes=0
install_one "${SYSTEMD_SRC}/pm-utils-hooks-watchdog.sh" "${LIBEXEC_DIR}/pm-utils-hooks-watchdog.sh" "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-pm-utils.service"   "${SYSTEMD_DIR}/selfdef-pm-utils.service"   "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-pm-utils.timer"     "${SYSTEMD_DIR}/selfdef-pm-utils.timer"     "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_PMUTILS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-pm-utils.timer" -- \
        systemctl enable --now selfdef-pm-utils.timer || true
fi

emit_status "ok" "pm-utils-hooks-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
