...
changes=0
install_one "${SYSTEMD_SRC}/openvpn-config-watchdog.sh" "${LIBEXEC_DIR}/openvpn-config-watchdog.sh" "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-openvpn.service"    "${SYSTEMD_DIR}/selfdef-openvpn.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-openvpn.timer"      "${SYSTEMD_DIR}/selfdef-openvpn.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-openvpn.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_OPENVPN_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-openvpn.timer" -- \
        systemctl enable --now selfdef-openvpn.timer || true
fi

emit_status "ok" "openvpn-config-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
