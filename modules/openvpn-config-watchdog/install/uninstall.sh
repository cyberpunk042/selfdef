#!/usr/bin/env bash
# openvpn-config-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-openvpn.timer || true
    systemctl reset-failed selfdef-openvpn.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-openvpn.service" \
         "${SYSTEMD_DIR}/selfdef-openvpn.timer" \
         "${LIBEXEC_DIR}/openvpn-config-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "openvpn-config-watchdog removed=$removed (baseline preserved)"
