#!/usr/bin/env bash
# kernel-install-hooks-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/kernel-install-hooks-watchdog.sh" "${LIBEXEC_DIR}/kernel-install-hooks-watchdog.sh" "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-kernel-hooks.service"     "${SYSTEMD_DIR}/selfdef-kernel-hooks.service"     "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-kernel-hooks.timer"       "${SYSTEMD_DIR}/selfdef-kernel-hooks.timer"       "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-kernel-hooks.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_KHOOK_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-kernel-hooks.timer" -- \
        systemctl enable --now selfdef-kernel-hooks.timer || true
fi

emit_status "ok" "kernel-install-hooks-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
