...
changes=0
install_one "${SYSTEMD_SRC}/autofs-watchdog.sh"     "${LIBEXEC_DIR}/autofs-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-autofs.service" "${SYSTEMD_DIR}/selfdef-autofs.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-autofs.timer"   "${SYSTEMD_DIR}/selfdef-autofs.timer"   "0644" && changes=$((changes + 1)) || true
...
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_AUTOFS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-autofs.timer" -- \
        systemctl enable --now selfdef-autofs.timer || true
fi

emit_status "ok" "autofs-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
