...
if [[ ! -r "$_LIB" ]]; then
    logger -t selfdef-initramfs-hooks -- '{"tag":"selfdef-initramfs-hooks","severity":"alert","event":"module_lib_missing","profile":"'"$PROFILE"'"}'
    exit 1
fi
# shellcheck disable=SC1090
source "$_LIB"
if [[ "${SELFDEF_MODULE_LIB_VERSION:-0}" -lt 3 ]]; then
    logger -t selfdef-initramfs-hooks -- '{"tag":"selfdef-initramfs-hooks","severity":"alert","event":"module_lib_outdated","profile":"'"$PROFILE"'"}'
    exit 1
fi
