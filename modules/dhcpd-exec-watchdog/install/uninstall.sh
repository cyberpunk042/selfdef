...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-dhcpd-exec.timer || true
    systemctl reset-failed selfdef-dhcpd-exec.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-dhcpd-exec.service" \
         "${SYSTEMD_DIR}/selfdef-dhcpd-exec.timer" \
         "${LIBEXEC_DIR}/dhcpd-exec-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "dhcpd-exec-watchdog removed=$removed (baseline preserved)"
