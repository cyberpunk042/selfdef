#!/usr/bin/env bash
# bash-completion-watchdog — apply.
...
changes=0
install_one "${SYSTEMD_SRC}/bash-completion-watchdog.sh"     "${LIBEXEC_DIR}/bash-completion-watchdog.sh"     "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-bash-completion.service" "${SYSTEMD_DIR}/selfdef-bash-completion.service" "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-bash-completion.timer"   "${SYSTEMD_DIR}/selfdef-bash-completion.timer"   "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-bash-completion.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_BASHCOMP_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-bash-completion.timer" -- \
        systemctl enable --now selfdef-bash-completion.timer || true
fi

emit_status "ok" "bash-completion-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
