...
changes=0
install_one "${SYSTEMD_SRC}/ssh-authkeys-watchdog.sh"        "${LIBEXEC_DIR}/ssh-authkeys-watchdog.sh"        "0755" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-authkeys.service"    "${SYSTEMD_DIR}/selfdef-ssh-authkeys.service"    "0644" && changes=$((changes + 1)) || true
install_one "${SYSTEMD_SRC}/selfdef-ssh-authkeys.timer"      "${SYSTEMD_DIR}/selfdef-ssh-authkeys.timer"      "0644" && changes=$((changes + 1)) || true

DROPIN_DIR_SVC="${SYSTEMD_DIR}/selfdef-ssh-authkeys.service.d"
DROPIN_PROFILE="${DROPIN_DIR_SVC}/50-profile.conf"
mkdir -p "$DROPIN_DIR_SVC"
tmp_dropin="$(mktemp)"
cat > "$tmp_dropin" <<EOF
[Service]
Environment=SELFDEF_AUTHKEYS_PROFILE=${PROFILE}
EOF
...
if [[ "$changes" -gt 0 ]] && [[ "$DRY_RUN" != "1" ]]; then
    ...
    run "enable + start selfdef-ssh-authkeys.timer" -- \
        systemctl enable --now selfdef-ssh-authkeys.timer || true
fi

emit_status "ok" "ssh-authkeys-watchdog profile=$PROFILE changes=$changes (first scan baselines)"
