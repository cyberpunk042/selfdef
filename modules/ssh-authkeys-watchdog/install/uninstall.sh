#!/usr/bin/env bash
# ssh-authkeys-watchdog — uninstall.
...
if command -v systemctl >/dev/null 2>&1 && [[ "$DRY_RUN" != "1" ]]; then
    run "disable + stop timer" -- systemctl disable --now selfdef-ssh-authkeys.timer || true
    systemctl reset-failed selfdef-ssh-authkeys.service 2>/dev/null || true
fi

removed=0
for f in "${SYSTEMD_DIR}/selfdef-ssh-authkeys.service" \
         "${SYSTEMD_DIR}/selfdef-ssh-authkeys.timer" \
         "${LIBEXEC_DIR}/ssh-authkeys-watchdog.sh"; do
    if [[ -f "$f" ]]; then
...
emit_status "ok" "ssh-authkeys-watchdog removed=$removed (baseline preserved)"
