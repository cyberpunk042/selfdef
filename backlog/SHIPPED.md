...
| four-watchdog observability surface | `packaging/scripts/four-watchdog-textfile.sh` (NEW production wrapper) — atomic-tempfile + rename writer of Prometheus node_exporter textfile gauges from the daemon's `/v1/alerts` 9-row response (consumed via `selfdefctl alerts --json`). Honest-offline sentinel gauge `selfdef_four_watchdog_textfile_emit_failed=1` when selfdefd is unreachable / selfdefctl unavailable / response malformed — never silently emits a zeroed textfile that would mask a wedged daemon as healthy. Emits 4 canonical gauge series: `selfdef_four_watchdog_worst_severity` (rollup), `selfdef_four_watchdog_severity{alert,ms,series}` (per-alert with milestone-family labels enabling Grafana grouping across MS046/MS047/MS044/MS048), `selfdef_four_watchdog_last_run_unix` (observer-freshness for the observer-silent alert family), `selfdef_four_watchdog_textfile_emit_failed` (sentinel). Exit codes 0/1/2 mirror the severity ladder (matches the cli-mirror + m060 doctor convention) | this commit | 23 contract tests in `crates/selfdef-daemon/tests/four_watchdog_doctor_unit_contract.rs` (7 service-unit assertions, 5 timer assertions, 11 wrapper assertions covering shebang/strict-mode/JSON-input/atomic-write/failure-sentinel/canonical-gauges/severity-ladder/per-alert-labels/IPS-spine-anchor-MS046+MS047+MS044+MS048 documentation) | 3 new Cargo.toml deb-assets rows (the `.sh` + `.service` + `.timer`) |
| four-watchdog systemd doctor unit + timer | `packaging/systemd/selfdef-four-watchdog-doctor.service` + `packaging/systemd/selfdef-four-watchdog-doctor.timer` — 60s cadence one-shot pairing with the wrapper above. Same R171 hardening posture as the sibling cli-mirror-doctor + m060-doctor units (User=selfdef + 12-clause defense-in-depth + PrivateNetwork=true + ReadWritePaths restricted to `/var/lib/node_exporter/textfile_collector` + READ-ONLY everywhere else including `/var/lib/selfdef` to enforce observer-vs-producer separation). Timer: OnBootSec=90s (90s offset from sibling -doctor timers at 60s and 70s to avoid boot-time pile-up), OnUnitActiveSec=60s, RandomizedDelaySec=10s for fleet-wide decorrelation. SuccessExitStatus=0 1 2 — the wrapper's WARN/CRITICAL exit codes are signals captured in the metric, not unit failure | this commit | 12 of the 23 contract tests above (7 service + 5 timer assertions) | bundled into the same 3 Cargo.toml deb-assets rows |
| module-catalog observer wrapper | `packaging/scripts/selfdef-modules-textfile.sh` (NEW production wrapper) — calls `selfdefctl modules list --json` + emits 5 canonical Prometheus textfile gauges atomically: `selfdef_modules_total` (catalog count rollup), `selfdef_modules_by_category{category=…}` (per-category roll-up via `jq group_by(.category)`), `selfdef_modules_by_phase{phase=…}` (per-phase roll-up: pre/main/post), `selfdef_modules_last_run_unix` (observer freshness for the chain-wide observer-silent alert family), `selfdef_modules_textfile_emit_failed` (honest-offline sentinel matching the four-watchdog wrapper convention). Honest-offline: when selfdefctl/jq absent OR daemon-side module list fails OR JSON envelope malformed, emit sentinel gauge `selfdef_modules_textfile_emit_failed=1` — never silently emit a zeroed textfile that would mask a wedged daemon as "host with zero modules". 188+ shipped modules across hardening / observability / etc categories now Prometheus-visible | this commit | 22 contract tests in `crates/selfdef-daemon/tests/modules_textfile_unit_contract.rs` (7 service-unit assertions, 5 timer assertions, 10 wrapper assertions covering shebang/strict-mode/JSON-input/atomic-write/failure-sentinel/canonical-gauges/per-category-label/JSON-shape-validation/honest-offline-doctrine/SELFDEF_MODULES_DIR-override) | 3 new Cargo.toml deb-assets rows (the `.sh` + `.service` + `.timer`) |
| module-catalog systemd unit + timer | `packaging/systemd/selfdef-modules-textfile.service` + `packaging/systemd/selfdef-modules-textfile.timer` — 60s cadence one-shot pairing with the wrapper above. Same R171 hardening posture as sibling units (User=selfdef + 12-clause defense-in-depth + PrivateNetwork=true + ReadWritePaths restricted to `/var/lib/node_exporter/textfile_collector` + READ-ONLY everywhere else). Timer: OnBootSec=120s (120s offset above the 3 sibling -doctor timers at 60s/70s/90s to avoid boot-time pile-up), OnUnitActiveSec=60s, RandomizedDelaySec=10s. SuccessExitStatus=0 (no severity ladder; modules either enumerate or fail — distinct from doctor observers which use SuccessExitStatus=0 1 2). After=local-fs.target | this commit | 12 of the 22 contract tests above (7 service + 5 timer assertions) | bundled into the same 3 Cargo.toml deb-assets rows |
| **NEW this commit** daemon process-state observer wrapper | `packaging/scripts/selfdef-daemon-process-textfile.sh` (NEW production wrapper) — resolves selfdefd PID via `systemctl show -p MainPID` + reads `/proc/<pid>/status` + `/proc/<pid>/stat` + `/proc/<pid>/fd/` for live process state. Emits 8 canonical Prometheus textfile gauges atomically: `selfdef_daemon_process_memory_rss_bytes` (VmRSS), `selfdef_daemon_process_memory_vsize_bytes` (VmSize), `selfdef_daemon_process_open_fds` (count of /proc/<pid>/fd entries), `selfdef_daemon_process_threads` (status Threads), `selfdef_daemon_process_uptime_seconds` (computed via clock-tick math: /proc/uptime - start_ticks/CLK_TCK), `selfdef_daemon_process_restart_count` (systemd NRestarts property, declared as TYPE counter for rate() correctness), `selfdef_daemon_process_last_run_unix` (observer freshness), `selfdef_daemon_process_textfile_emit_failed` (honest-offline sentinel). Honest-offline: when systemctl absent OR daemon MainPID=0 (not running) OR /proc/<pid>/ inaccessible, emit sentinel gauge — never silently emit zeroed memory + FD gauges that would mask a crashed daemon as "healthy with 0 RAM". Operator override via SELFDEF_DAEMON_UNIT for non-standard unit names. Real IPS-host operator visibility: memory leak detection + FD exhaustion + restart loop surveillance for the daemon that defends the host | this commit | 24 contract tests in `crates/selfdef-daemon/tests/daemon_process_textfile_unit_contract.rs` (8 service-unit assertions including After=selfdefd.service ordering, 5 timer assertions, 11 wrapper assertions covering shebang/strict-mode/systemctl-PID-resolution/proc-fs-reads/atomic-write/MainPID=0-sentinel/all-8-canonical-gauges/NRestarts-via-systemctl/honest-offline-doctrine/SELFDEF_DAEMON_UNIT-override/counter-type-declaration) | 3 new Cargo.toml deb-assets rows |
| **NEW this commit** daemon process-state systemd unit + timer | `packaging/systemd/selfdef-daemon-process-textfile.service` + `packaging/systemd/selfdef-daemon-process-textfile.timer` — 60s cadence one-shot pairing with the wrapper above. Same R171 hardening posture as sibling units (User=selfdef + 12-clause defense-in-depth + PrivateNetwork=true + ReadWritePaths restricted to `/var/lib/node_exporter/textfile_collector` + READ-ONLY everywhere else). Service: After=local-fs.target selfdefd.service (orders after daemon so PID lookup succeeds). Timer: OnBootSec=150s (150s offset above the 4 sibling observer timers at 60s/70s/90s/120s — 5 observer timers, 5 distinct boot offsets, prevents synchronized first-fire after boot). OnUnitActiveSec=60s, RandomizedDelaySec=10s. SuccessExitStatus=0 (no severity ladder; process state either reads or doesn't) | this commit | 13 of the 24 contract tests above (8 service + 5 timer assertions) | bundled into the same 3 Cargo.toml deb-assets rows |
| **NEW** E0275 / R06323 / F03190 — Tetragon metric-name contract (closes finding **F-2026-052**) | The dashboard's 4 upstream-Tetragon panels (`tetragon_events_total`, `tetragon_msg_sigkill_total`, `tetragon_process_cache_size`, `tetragon_map_errors_total`) were pinned only in README prose — a Tetragon rename rendered a panel flat with no error (silent-fail-open observability). Now pinned in a machine-checkable contract `modules/observability/assets/contracts/tetragon-metrics.toml` (series + `verified_tetragon_version >=1.0.0,<2.0.0` + canonical_source). Three lines of defense per **SDD-079**: (1) CI test `tests/observability/test_tetragon_metric_name_contract.py` locks dashboard↔contract↔SDD↔README in lockstep — character-for-character promql equality + no-unpinned-tetragon-series drift catch; (2) `modules/observability/install/check.sh` operability probe (opt-in `SELFDEF_OBSERVABILITY_PROBE_TETRAGON=1`, warn-only) scrapes the live Tetragon `/metrics` endpoint and warns per-series-absent, turning a silent flat panel into a loud check warning; (3) declared version window documents the upstream compatibility gate. Findings ledger row flipped partial→**closed**. Phase-3 follow-up (fail-closed binary-version enforcement from the tetragon `requires` block) tracked in SDD-079 D-1 | this commit | 8 contract tests in `tests/observability/test_tetragon_metric_name_contract.py` (contract-shape ×3, dashboard↔contract lockstep ×2, doc lockstep ×3) | new module asset `assets/contracts/tetragon-metrics.toml` (deb-shipped with the module tree); no Cargo deb-asset delta (module-side asset) |
