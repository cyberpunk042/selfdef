...
| MS011 | partial | 13 of 13 Z-vectors at probe/discovery level + Z-1 **functionally complete** via SDD-056 + Z-13 SD-R87 topological-sort install plan via `GET /v1/modules/install-plan` (Kahn's algorithm) + Z-13 SD-R86 hardware-gate enrichment **closed** via SDD-057 (selfdef-hardware-requirements crate split + `/v1/modules/install-options` extended to evaluate gate against probed capabilities → 4-way classification: ready / blocked-by-missing-deps / blocked-by-hardware / needs-review). Multi-commit follow-up arcs (NOT blocking the discovery layer): Z-2 LM Studio shell-out invocation, Z-3 apply + revert mutation surfaces, Z-12 multi-tier REPL Python implementation completion |
