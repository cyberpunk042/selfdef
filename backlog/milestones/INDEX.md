...
| MS002 | partial | collector crates exist; eBPF collector + tetragon collector + eventstream + journald + **auditd collector with AVC support** (parses `avc: denied/granted` SELinux/SMACK decisions, maps to ClassUid::PROCESS_ACTIVITY + ProcessActivity::Open + T1083 attack tag on denials) shipped. 3 more eBPF programs deferred per SDD-032 (eBPF substrate + Tetragon policy ledger). Multi-line SYSCALL+EXECVE auditd parsing deferred. |
...
| MS013 | partial | 49 SDDs at `implemented` (+ 1 living, 1 review, 4 scoping, 5 draft = 60 total in `docs/sdd/`). Net+22 ratifications since pre-session count. Ongoing as production catches up to spec authoring; SDD-026 + SDD-058 are this batch's promotions |
...
| MS043 | partial | IPS operator surface — CLI shipped (selfdefctl 14 top-level verbs + many subverbs, now including `inference-backends {show,version}`); dashboard at 17 panels under SDD-056 8-tab nav (composite-health + 4-watchdog + modules + audit-chains + alerts + hardware + network + storage + raid + gpu + cpu + flex-profile + inference-backends + control + findings) + **operator-facing per-panel visibility menu** ("View ▾" → localStorage-persisted `selfdef.hiddenPanels` set; closes the operator's "everything can be turned on and off" verbatim UX requirement); MS045 coherence harness validates surface. Operator-pull TUI deferred. |
