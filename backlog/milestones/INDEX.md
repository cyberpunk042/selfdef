...
| MS011 | partial | 13 of 13 Z-vectors shipped at probe/discovery level (Z-1 panel-nav strip; Z-2 `GET /v1/inference-backends` probe for llama.cpp / vllm / bitnet.cpp / unsloth; Z-3 `selfdef-flex-profile` crate + CLI + HTTP discovery; Z-4 CPU mode; Z-5 GPU watt; Z-6 composite health; Z-7 network; Z-8 install-paths; Z-9 RAID; Z-10 storage; Z-11 MCP foundation; Z-13 SD-R83 + SD-R86 dep-readiness). Multi-commit follow-up arcs (NOT blocking the discovery layer): Z-1 full 8-tab restructure, Z-2 LM Studio shell-out invocation (the probe surfaces installed state — shell-out invocation through SDD-050 is a separate Stage-3 round), Z-3 apply + revert mutation surfaces, Z-12 multi-tier REPL Python implementation, Z-13 SD-R87 topological install plan + SD-R86 hardware-gate enrichment |
