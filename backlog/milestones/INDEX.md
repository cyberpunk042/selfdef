...
| MS011 | partial | 13 of 13 Z-vectors at probe/discovery level + Z-1 **functionally complete** via SDD-056 + Z-13 SD-R87 topological-sort install plan + Z-13 SD-R86 hardware-gate enrichment via SDD-057 + Z-3 apply/revert mutation surfaces via POST `/v1/flex-profile/{apply,revert}` (atomic disk persistence; operator-gating through SDD-043 commit-authority deferred as separate caller-integration arc) + Z-2 SD-R(unnumbered) `selfdefctl inference-backends version <backend>` invocation seed + Z-12 SD-R102 operator-macro auto-load (`$SELFDEF_REPL_MACROS` > XDG > `~/.config/selfdef/repl-macros.py`). Multi-commit follow-up arcs (NOT blocking the discovery layer): Z-2 full module-driven install pipeline, Z-12 dashboard REPL pop-out UI |
