...
**Source addendum (backward-sweep, 2026-05-20)**: per `~/devops-solutions-information-hub/wiki/log/2026-05-20-avx-plus-plus-dump-tail-backward-sweep-review.md`, the dump tail at **lines 18000-18100** elaborates the six profiles with per-profile **scheduling-rule tuples** (favor-latency / oracle-verification / tests-required / sandbox-first / wide-branch-search / etc.). These tuples extend — do NOT supplant — the authority envelopes documented above. The scheduling tuples are operationalized by MS048 (Goldilocks Scheduler) via `selfdef-scheduler::ProfileRules::for_profile()`; this milestone retains ownership of the **authority envelope** projection (L0..L6 clipping per profile). Both surfaces compose: MS040 says "what mutations is this profile allowed to commit"; MS048 says "given that envelope, what hardware tier + scheduling discipline applies."
